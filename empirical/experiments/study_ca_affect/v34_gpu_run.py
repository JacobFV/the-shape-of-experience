"""V34 GPU Runner — Φ-Inclusive Fitness

Does selecting for integration directly increase the success rate?

Usage:
    python v34_gpu_run.py smoke
    python v34_gpu_run.py run --seed 42
    python v34_gpu_run.py all
"""
...
from v34_evolution import generate_v34_config, run_v34


def run_smoke():
    print("V34 SMOKE TEST (N=32, M=32, K=4, 3 cycles, alpha=2.0)")
    print("=" * 70)
    cfg = generate_v34_config(
        N=32, M_max=32,
        ...
        drought_every=0,
        phi_fitness_alpha=2.0,
    )
    print(f"  n_params: {cfg['n_params']}")
    result = run_v34(seed=42, cfg=cfg, output_dir='/tmp/v34_smoke')
    print(f"\nSmoke test complete!")
    return result


def run_single(seed, output_base='results', n_cycles=30, steps_per_cycle=5000):
    output_dir = f'{output_base}/v34_s{seed}'
    cfg = generate_v34_config(
    ...
    print(f"\n{'='*70}")
    print(f"V34 Φ-INCLUSIVE FITNESS SEED {seed}: alpha={cfg['phi_fitness_alpha']}")
    print(f"  n_params: {cfg['n_params']}")
    print(f"  Output: {output_dir}")
    print(f"{'='*70}\n")
    return run_v34(seed=seed, cfg=cfg, output_dir=output_dir)


def run_all(output_base='results', **kwargs):
    """Run 10 seeds for comparison with V27/V31."""
    ...
    phi_rob_corrs = [r['summary']['phi_rob_correlation'] for r in results.values()]
    ...
    print(f"\n{'='*70}")
    print(f"V34 ALL {n} SEEDS COMPLETE — {elapsed:.0f}s")
    print(f"{'='*70}")
    ...
    print(f"  Φ-Rob corr: {np.mean(phi_rob_corrs):.3f} "
          f"({'OK' if np.mean(phi_rob_corrs) > 0 else 'GOODHART'})")

    v27_baseline = 0.090
    t_stat = (np.mean(mean_phis) - v27_baseline) / (
        np.std(mean_phis) / np.sqrt(n))
    print(f"  t-stat vs V27: {t_stat:.2f}")
    ...
    summary = {
        ...
        'statistics': {
            ...
            'phi_rob_corr_mean': float(np.mean(phi_rob_corrs)),
            't_stat_vs_v27': float(t_stat),
        },
    }
    with open(f'{output_base}/v34_summary.json', 'w') as f:
        json.dump(summary, f, indent=2)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='V34 Φ-Inclusive Fitness')
