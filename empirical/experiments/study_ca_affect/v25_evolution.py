"""V25: Evolution Loop — Predator-Prey on Structured Landscape

Separate evolution for prey and predators. Prey are selected on
resource harvesting efficiency. Predators are selected on prey
consumption. Both experience drought bottlenecks.

Key difference from V20b: offspring are placed near resource patches
(not random), and prey/predators are evolved independently.
"""
...
from v25_substrate import (
    generate_v25_config, init_v25, make_chunk_runner,
    compute_phi_hidden, compute_robustness, extract_snapshot,
)


# ---------------------------------------------------------------------------
# Fitness computation (type-specific)
# ---------------------------------------------------------------------------
...
def compute_pred_fitness(survival_time, alive):
    """Predator fitness: pure survival. Surviving = eating enough prey."""
    return survival_time * alive.astype(jnp.float32)


# ---------------------------------------------------------------------------
# Tournament selection (per-type)
# ---------------------------------------------------------------------------

def tournament_selection_typed(params, fitness, alive, type_mask, key, cfg):
    """Tournament selection within a type (prey or predators).

    type_mask: bool array, True for agents of this type.
    Only competes and reproduces among same type.
    """
    M = params.shape[0]
    P = params.shape[1]
    ...
    # Fill non-elite slots of this type
...
def run_cycle_with_metrics(state, chunk_runner, cfg, regen_override=None):
    ...
    # Combine: prey use prey_fitness, preds use pred_fitness
...
def run_v25(seed, cfg, output_dir):
    """Run V25 evolution for one seed."""
    os.makedirs(output_dir, exist_ok=True)

    key = jax.random.PRNGKey(seed)
    state, key, patch_centers = init_v25(seed, cfg)

    # Store patch centers separately (not in state dict — would break lax.scan)
    ...
    for cycle in range(cfg['n_cycles']):
        ...
        fname = os.path.join(output_dir, f'v25_s{seed}_progress.json')
    ...
    with open(os.path.join(output_dir, f'v25_s{seed}_results.json'), 'w') as f:
