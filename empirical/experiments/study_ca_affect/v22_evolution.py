"""V22: Evolution Loop — Intrinsic Predictive Gradient

Evolutionary loop for V22 neural agents with within-lifetime SGD.

Key differences from V21:
  - Genome/phenotype distinction: phenotype = genome + accumulated SGD updates
  - At cycle start: phenotype ← genome (reset learned changes)
  - At reproduction: only genome inherited (Baldwinian) or phenotype (Lamarckian)
  - Prediction MSE tracking per cycle (within-lifetime learning curve)
  - Phenotype drift metrics (how far phenotype drifts from genome)
"""
...
# Reuse V20 evolution machinery (fitness + selection work on genome arrays directly)
from v20_evolution import (
    compute_fitness, tournament_selection,
)


def rescue_population_v22(state, key, cfg, min_pop=10):
    """V22-specific rescue: uses genomes instead of params."""
    ...
    new_state['phenotypes'] = jnp.array(new_genomes)  # phenotype = genome for rescued agents
...
# ---------------------------------------------------------------------------
# Tick usage metrics (same as V21)
# ---------------------------------------------------------------------------
...
def compute_tick_usage(state, cfg):
    ...
    # Use GENOMES for tick usage (phenotype may have drifted)
...
def run_cycle_with_metrics(state, chunk_runner, cfg, stress=False, regen_override=None):
    """Run one cycle, collecting V22 metrics including pred MSE."""
    ...
    # Prediction MSE trajectory within this cycle (early vs late)
...
def run_v22(seed, cfg, output_dir):
    """Run V22 evolution for one seed.

    Key difference from V21: genome/phenotype distinction.
    - Phenotype ← genome at cycle start (reset within-lifetime learning)
    - SGD updates phenotype during cycle
    - Tournament selection operates on genomes
    - Baldwinian: only genome inherited; Lamarckian: phenotype → genome option
    """
    os.makedirs(output_dir, exist_ok=True)

    key = jax.random.PRNGKey(seed)
    state, key = init_v22(seed, cfg)

    # Warmup JIT
    print(f"Warming up JIT (seed {seed})...")
    t0 = time.time()
    chunk_runner = make_v22_chunk_runner(cfg)
    ...
    for cycle in range(cfg['n_cycles']):
        ...
        # Tick usage, sync decay, lr stats (from genomes)
        ...
        # === LAMARCKIAN OPTION: copy phenotype back to genome before selection ===
        ...
        # Rescue if needed
        state, key = rescue_population_v22(state, key, cfg)
        ...
        with open(os.path.join(output_dir, f'v22_s{seed}_progress.json'), 'w') as f:
    ...
    with open(os.path.join(output_dir, f'v22_s{seed}_results.json'), 'w') as f:
