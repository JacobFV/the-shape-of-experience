"""V15 Evolution: Temporal Lenia with Memory Channels.

Extends V14 evolution with:
- Memory lambda mutation (EMA time constants)
- Oscillating resource patches (regen mask updates each cycle)
- All V14 features (chemotaxis param mutation, foraging metrics)
"""
...
def mutate_v15_params(tau, gate_beta, coupling, chemotaxis_strength,
                      motor_sensitivity, motor_threshold,
                      memory_lambdas,
                      rng, coupling_bandwidth=3):
    """Mutate V15 parameters: V14 params + memory lambdas."""
    ...
    for i, lam in enumerate(memory_lambdas):
        # Log-space mutation (keeps lambdas positive, respects scale)
...
def evolve_v15(config=None, n_cycles=30, steps_per_cycle=5000,
    ...
    """V15: Evolution with memory channels and oscillating resources."""
    if config is None:
    ...
    print("=" * 60)
    print(f"V15 TEMPORAL LENIA EVOLUTION (C={C}, N={N})")
    print("=" * 60)
    ...
    print(f"  Patch shift:     {config['patch_shift_period']} steps")
    ...
    for cycle in range(n_cycles):
        ...
        print(f"C{cycle:02d} | pat={n_final:4d} | mort={mortality:.0%} | "
              ...
              f"λ=[{','.join(f'{l:.3f}' for l in memory_lambdas)}] | "
              f"{elapsed:.0f}s")
    ...
    if cycle_stats:
        ...
        print(f"  Eta:          {first['chemotaxis_strength']:.2f} -> {last['chemotaxis_strength']:.2f}")
