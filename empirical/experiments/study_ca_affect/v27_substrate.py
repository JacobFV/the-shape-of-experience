"""V27: Nonlinear MLP Prediction Head — Testing the Linear Readout Bottleneck

V22-V24 showed: within-lifetime gradient learning works, but integration
(Φ) does not increase regardless of prediction target (energy, environment,
value). The corrected hidden state analysis (post-bug-fix) reveals eff_rank
5-7 — agents have ROOM for cross-component coordination but aren't forced
into it.

Hypothesis: the bottleneck is the LINEAR prediction head.

With linear readout W∈R^{H×1}:
    pred = h @ W + b
    ∂L/∂h_i = 2(pred - target) * W_i
    → gradient for unit i depends ONLY on W_i
    → evolution can zero out most W_i, using a subset of hidden dims

With nonlinear MLP readout:
    z = tanh(h @ W1 + b1)       # H → H/2
    pred = z @ W2 + b2           # H/2 → 1
    ∂L/∂h_i = 2(pred - target) * Σ_k W2_k * (1-tanh²(z_k)) * W1_{k,i}
    → gradient for unit i depends on ALL hidden units through z_k
    → creates coupling pressure: no unit can be zeroed without affecting all others

This is the most direct test of the identified bottleneck. Everything else
is identical to V22 (same GRU, same ticks, same SGD, same environment).

Pre-registered predictions:
  P1: MSE still decreases within lifetime (gradient works through MLP)
  P2: Mean Φ > V22's 0.097 (nonlinear readout forces cross-component computation)
  P3: Mean robustness > V22's 0.981 (integrated representation improves survival)
  P4: Effective rank of hidden state at cycle 29 > V22 (more dims actively used)
  P5: Prediction Jacobian has rank > 1 (multiple hidden dims contribute)

Falsification criteria:
  - P2 fails (Φ ≤ 0.097): bottleneck is NOT the linear readout
  - MLP collapses to linear regime (W1 entries small, tanh ≈ identity): need stronger nonlinearity
  - Gradient instability (phenotype drift >> V22): MLP amplifies gradient noise
"""
...
def generate_v27_config(**kwargs):
    """Generate V27 configuration with MLP prediction head."""
    cfg = {
        # Grid
        'N': 128,
        'M_max': 256,
...
def _param_shapes(cfg):
    ...
    O = cfg['n_actions']
    K = cfg['K_max']
    PH = cfg['predict_hidden']  # MLP hidden dim
    return {
        # V20 base params
        'embed_W': (obs_flat, E),
        'embed_b': (E,),
        ...
        'tick_weights': (K,),
        'sync_decay_raw': (1,),
        # V27 MLP prediction params (replaces V22's linear head)
        'predict_W1': (H, PH),
        'predict_b1': (PH,),
        'predict_W2': (PH, 1),
        'predict_b2': (1,),
...
# ---------------------------------------------------------------------------
# Agent multi-tick forward pass with MLP prediction (single agent)
# ---------------------------------------------------------------------------

def agent_multi_tick_v27(obs_flat, hidden, sync_matrix, params_flat, cfg):
    """Run K_max GRU ticks for one agent, returning MLP prediction.

    Same as V22 but prediction uses 2-layer MLP instead of linear head:
        z = tanh(h @ W1 + b1)
        pred = z @ W2 + b2
    ...
    # === V27: MLP prediction from final hidden state ===
...
agent_multi_tick_v27_batch = jax.vmap(
    agent_multi_tick_v27, in_axes=(0, 0, 0, 0, None)
)


# ---------------------------------------------------------------------------
# Prediction gradient (single agent) — MLP version
# ---------------------------------------------------------------------------
...
def _prediction_loss(phenotype_flat, obs_flat, hidden, sync_matrix, cfg, actual_delta):
    ...
    Uses MLP prediction head — gradient flows through tanh nonlinearity,
    creating coupling between all hidden units.
    """
...
# ---------------------------------------------------------------------------
# V27 environment step
# ---------------------------------------------------------------------------

def _env_step_v27_inner(state, cfg):
    """V27 env step: multi-tick with MLP prediction."""
    N = cfg['N']
...
def make_v27_chunk_runner(cfg):
    """Create JIT-compiled chunk runner with MLP prediction gradient."""
    def scan_body(state, step_idx):
        ...
        new_state, obs, predictions = _env_step_v27_inner(state, cfg)
    ...
    @jax.jit
    def run_chunk(state):
        """Run chunk_size steps with MLP gradient learning."""
...
# ---------------------------------------------------------------------------
# Tick weight and sync decay extraction
# ---------------------------------------------------------------------------
...
def extract_predict_mlp_np(params, cfg):
    """Extract MLP prediction head weights from (M, P) params array."""
    H = cfg['hidden_dim']
    PH = cfg['predict_hidden']
    offset_W1 = _param_offset(cfg, 'predict_W1')
    offset_b1 = _param_offset(cfg, 'predict_b1')
    offset_W2 = _param_offset(cfg, 'predict_W2')
    offset_b2 = _param_offset(cfg, 'predict_b2')

    W1 = np.array(params)[:, offset_W1:offset_W1 + H * PH].reshape(-1, H, PH)  # (M, H, PH)
    b1 = np.array(params)[:, offset_b1:offset_b1 + PH]  # (M, PH)
    W2 = np.array(params)[:, offset_W2:offset_W2 + PH]  # (M, PH)
    b2 = np.array(params)[:, offset_b2]  # (M,)
    return W1, b1, W2, b2
...
def init_v27(seed, cfg):
    """Initialize V27 state — same as V22 but with more prediction params."""
