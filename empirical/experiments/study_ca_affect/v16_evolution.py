"""V16 Evolution: Plastic Lenia with Within-Lifetime Learning.

Extends V15 evolution with:
- Learning rate mutation (log-space)
- Coupling decay rate mutation (log-space)
- Coupling field persistence across steps (but reset each cycle — within-generation learning)
- Plasticity metrics: coupling divergence, spatial variance of coupling
"""
...
def mutate_v16_params(tau, gate_beta, coupling, chemotaxis_strength,
                      motor_sensitivity, motor_threshold,
                      memory_lambdas,
                      learning_rate, decay_rate_coupling,
                      rng, coupling_bandwidth=3):
    """Mutate V16 parameters: V15 params + plasticity params."""
    ...
    # V16 plasticity (log-space)
    log_lr = np.log(learning_rate + 1e-8)
    log_lr_new = log_lr + 0.3 * float(random.normal(keys[7], ()))
    lr_new = float(np.clip(np.exp(log_lr_new), 1e-6, 0.01))

    log_dr = np.log(decay_rate_coupling + 1e-8)
    log_dr_new = log_dr + 0.3 * float(random.normal(keys[8], ()))
    dr_new = float(np.clip(np.exp(log_dr_new), 1e-7, 0.01))
...
def evolve_v16(config=None, n_cycles=30, steps_per_cycle=5000,
    ...
    """V16: Evolution with within-lifetime learning."""
    if config is None:
    ...
    print("=" * 60)
    print(f"V16 PLASTIC LENIA EVOLUTION (C={C}, N={N})")
    print("=" * 60)
    print(f"  Memory channels: {config['memory_channels']}")
    print(f"  Learning rate:   {lr}")
    print(f"  Coupling decay:  {dr}")
    ...
    # Initial coupling field stats
    cf_np = np.array(coupling_field)
    cf_spatial_var = float(cf_np.var(axis=(2, 3)).mean())
    print(f"  Coupling spatial variance: {cf_spatial_var:.4f}\n")
    ...
    # Store initial coupling for divergence measurement
    ...
    for cycle in range(n_cycles):
        ...
        print(f"C{cycle:02d} | pat={n_final:4d} | mort={mortality:.0%} | "
              ...
              f"cf_var={coupling_spatial_var:.3f} | {elapsed:.0f}s")

        if post_cycle_callback:
            post_cycle_callback(cycle, stats, grid, resource, coupling_field, config)
    ...
    result = {
        ...
        'learning_rate': lr,
        'decay_rate_coupling': dr,
        'config': config,
        'condition': 'v16_plastic',
    }
    ...
    if cycle_stats:
        ...
        print(f"  Coupling var:   {first['coupling_spatial_var']:.3f} -> {last['coupling_spatial_var']:.3f}")
