"""V30: Evolution Loop — Dual Prediction (Self + Social)

Fork of V29 evolution with V30 substrate (dual prediction targets).
Same evolution loop, dual gradient signal.
"""
...
from v30_substrate import (
    generate_v30_config, init_v30, make_v30_chunk_runner,
...
def run_cycle_with_metrics(state, chunk_runner, cfg, regen_override=None):
    ...
    n_alive_samples = []
    pred_mse_samples = []
    pred_mse_self_samples = []
    pred_mse_social_samples = []
    ...
    for chunk_i in range(n_chunks):
        ...
        pred_mse_self_samples.append(float(np.mean(chunk_metrics['pred_mse_self'])))
        pred_mse_social_samples.append(float(np.mean(chunk_metrics['pred_mse_social'])))
    ...
    mean_pred_mse_self = float(np.mean(pred_mse_self_samples))
    mean_pred_mse_social = float(np.mean(pred_mse_social_samples))
    ...
    return state, {
        ...
        'mean_pred_mse_self': mean_pred_mse_self,
        'mean_pred_mse_social': mean_pred_mse_social,
...
def run_v30(seed, cfg, output_dir):
    """Run V30 dual prediction evolution."""
    os.makedirs(output_dir, exist_ok=True)

    print(f"\n{'='*60}")
    print(f"V30 DUAL PREDICTION (self + social) — seed={seed}")
    print(f"  n_params={cfg['n_params']}")
    print(f"{'='*60}")

    key = jax.random.PRNGKey(seed)
    state, key = init_v30(seed, cfg)

    print("Warming up JIT...")
    t0 = time.time()
    chunk_runner = make_v30_chunk_runner(cfg)
    ...
    for cycle in range(cfg['n_cycles']):
        ...
        print(
            ...
            f"mse_s={metrics['mean_pred_mse_self']:.4f} "
            f"mse_n={metrics['mean_pred_mse_social']:.4f} | "
        ...
        progress = {
            'seed': seed,
            'experiment': 'v30_dual_prediction',
            'cycle': cycle,
        ...
        with open(os.path.join(output_dir, f'v30_s{seed}_progress.json'), 'w') as f:
    ...
    results = {
        ...
        'summary': {
            ...
            'mean_pred_mse_self': float(np.mean([c['mean_pred_mse_self'] for c in cycle_results])),
            'mean_pred_mse_social': float(np.mean([c['mean_pred_mse_social'] for c in cycle_results])),
            'final_pop': cycle_results[-1]['n_alive_end'],
        },
    }

    with open(os.path.join(output_dir, f'v30_s{seed}_results.json'), 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\n  [V30 s{seed}] Done. Mean Phi={results['summary']['mean_phi']:.3f}, "
          f"Max Phi={results['summary']['max_phi']:.3f}, "
