"""V15 GPU Run: Temporal Lenia evolution on Lambda Labs.

Extends V14 with memory channels (EMA dynamics) and oscillating resource
patches. Tracks memory lambda evolution, memory channel activation, and
all V14 metrics (foraging, Phi, robustness).

Usage:
    python v15_gpu_run.py --seed 42 --cycles 30 --output /home/ubuntu/results
"""
import sys
import os
import json
...
from v15_evolution import mutate_v15_params
...
def run_v15_gpu(n_cycles=30, C=16, N=128, sim_radius=5, seed=42,
    ...
    """Full V15 evolution with content output."""
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(f'{output_dir}/snapshots', exist_ok=True)

    print("=" * 70)
    print(f"V15 GPU RUN: C={C}, N={N}, {n_cycles} cycles, seed={seed}")
    print(f"  Memory: {memory_channels} channels")
    print(f"  Resource patches: {n_resource_patches}, shift={patch_shift_period}")
    ...
    # Calibrated resource dynamics (same as V13/V14)
    ...
    chunk = 50  # Fixed chunk size — ALL calls use this
    ...
    for cycle in range(n_cycles):
        ...
        while step < cycle_baseline_steps:
            # Update regen mask (patches shift over time)
            regen_mask = compute_patch_regen_mask(
        ...
        if fitness_scores:
            ...
            # Mutate evolvable params (V13 + V14 + V15)
            rng, k_mut = random.split(rng)
            (tau, gate_beta, coupling,
             eta, motor_sensitivity, motor_threshold,
             memory_lambdas) = mutate_v15_params(
                tau, gate_beta, coupling,
                eta, motor_sensitivity, motor_threshold,
                memory_lambdas, k_mut)
        ...
        print(f"C{cycle:02d} | pat={len(final_patterns):4d} | mort={mortality:.0%} | "
              ...
              f"eta={eta:.2f} disp={foraging['mean_displacement']:.1f} "
              f"lam=[{','.join(f'{l:.3f}' for l in memory_lambdas)}] "
              f"mem={memory_mean:.3f} | {elapsed:.0f}s")
        ...
        with open(f'{output_dir}/v15_progress.json', 'w') as f:
            json.dump({
    ...
    # ---- FINAL SAVE ----
    with open(f'{output_dir}/v15_final.json', 'w') as f:
        json.dump({
            'cycle_stats': cycle_stats,
            'phi_trajectory': all_phi_trajectories,
            'tau': float(tau),
    ...
    if cycle_stats:
        ...
        print(f"  Memory mean:  {first['memory_mean']:.4f} -> {last['memory_mean']:.4f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--patches', type=int, default=4,
                        help='Number of resource patches')
