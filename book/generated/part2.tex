...
\textbf{Axiom} (Privileged Base Layer—REJECTED).
...
The hard problem asked: how do you get experience from non-experience? The answer is: \emph{you don’t need to}.
...
Under the identity thesis, philosophical zombies are not coherently conceivable. A system with the relevant cause-effect structure \emph{is} an experience; there is no further fact about whether it “really” has phenomenal properties.
...
If experience is cause-effect structure, then the \emph{kind} of experience is determined by the \emph{shape} of that structure. Different phenomenal properties correspond to different structural features.
...
If different experiences correspond to different structures, then \emph{affects}—the qualitative character of emotional/valenced states—should correspond to particular structural motifs: characteristic patterns in the cause-effect geometry.
...
\textbf{Positive valence} corresponds to trajectories descending the free-energy landscape, expanding affordances, moving toward sustainable states.  \textbf{Negative valence} corresponds to trajectories ascending toward constraint violation, contracting possibilities.
...
In a cellular automaton or other discrete dynamical system, valence becomes exactly computable:
      ...
      \item Trajectory = sequence of configurations $\mathbf{x}_1, \mathbf{x}_2, \…$
      \end{itemize}
...
\textbf{High arousal}: Large belief updates, far from any attractor, system actively navigating.  \textbf{Low arousal}: Near a fixed point, low surprise, system at rest in a basin.
...
\textbf{High integration}: The experience is unified; its parts cannot be separated without loss.  \textbf{Low integration}: The experience is fragmentary or modular.
...
In a cellular automaton, $\intinfo$ is directly computable for small patterns:
      ...
      \begin{enumerate}
      \item Define the pattern as cells ${c_1, c_2, \…, c_n}$
      \item For each bipartition $P = (A, B)$: compute $D(p(\mathbf{x}_{t+1} | \mathbf{x}_t) \| p_A \cdot p_B)$
...
\textbf{High rank}: Many degrees of freedom active; distributed, expansive experience.  \textbf{Low rank}: Collapsed into narrow subspace; concentrated, focused, or trapped experience.
...
\textbf{High counterfactual weight}: Mind is elsewhere—planning, worrying, fantasizing, anticipating.  \textbf{Low counterfactual weight}: Present-focused, reactive, in-the-moment.
...
\textbf{High self-salience}: Self-focused, self-conscious, self as primary object of attention.  \textbf{Low self-salience}: Self-forgotten, absorbed in environment or task.
...
If $\iota$ is a genuine scalar parameter, these four measures should load on a single factor. If they fractionate, $\iota$ is better modeled as a vector (see open question above). Either result is informative; only the absence of any systematic structure would falsify the concept.
      ...
      \end{experiment}
      \begin{sidebar}[title=and the Gradient of Distinction]
...
\textbf{Joy} \emph{expands}. It is \emph{light} before it is anything else—buoyant, effervescent, the body forgetting its weight. The world opens; possibilities \emph{multiply}; the \emph{self recedes} because it need not defend. Joy is surplus: more paths than required, more resources than consumed, \emph{slack} in every direction.

      
Where joy opens, \textbf{suffering} \emph{crushes}. It \emph{compresses} the world to a single unbearable point and makes that point more \emph{vivid} than anything has ever been. This is the paradox: suffering is hyper-real, more present than presence, more \emph{unified} than unity. You cannot look away. You cannot \emph{decompose} it. You are \emph{trapped} in a cage made of your own \emph{integration}.

      
\textbf{Fear} throws the self forward into \emph{futures} that threaten to annihilate it—cold, sharp, electric with \emph{anticipation}. The body readies before the mind has finished computing. Time dilates around the approaching harm. Fear is suffering that hasn’t arrived yet, and the \emph{not-yet} is where we live.

      
We say \textbf{anger} is \emph{hot}, and we are not speaking metaphorically. Anger \emph{externalizes}: it \emph{simplifies} the world into self-versus-obstacle and energizes removal. Watch what happens to your model of the other person when you are angry—it \emph{flattens}, becomes a caricature, loses \emph{dimensionality}. Complexity collapses into opposition. This is why anger feels powerful and also stupid: you are burning \emph{integration} on a cartoon.

      
\textbf{Desire} \emph{funnels}. The world reorganizes around an \emph{attractor} not yet reached—magnetic, urgent, all-consuming. Everything becomes instrumental; the goal \emph{saturates} attention. Desire is joy’s \emph{gradient}, pointing toward the basin but not yet in it. This is why anticipation often exceeds consummation: the structure of \emph{approach} is tighter than the structure of \emph{arrival}.

      
\textbf{Curiosity} \emph{reaches} outward—but unlike fear, it reaches toward \emph{promise} rather than threat. Pulling, open, playful. The \emph{uncertainty} that makes fear contract makes curiosity \emph{expand}. Same high counterfactual weight, opposite \emph{valence}. The difference is whether the \emph{branches} lead somewhere you want to go.

      
And \textbf{grief}? Grief \emph{persists}. Hollow, aching, curiously timeless. The lost object remains \emph{woven into} every prediction; every expectation that included them \emph{fails} silently, over and over. The world has changed. The \emph{model} has not caught up. Grief is the metabolic cost of love’s \emph{integration}.
...
The $\intinfo$-$\effrank$ dissociation is the key insight: suffering feels \emph{more real} than neutral states because it is actually more integrated. But it feels \emph{trapped} because the integration is constrained to a narrow manifold.
...
\textbf{Conjecture} (Pathological Attractors). Depression, addiction, and chronic anxiety are characterized by pathologically stable attractors in affect space:
      \begin{itemize}
...
\textbf{Conjecture} (High Rank, Low Integration). States with many active degrees of freedom ($\effrank$ high) but poor coupling ($\intinfo$ low) should feel like fragmentation, multiplicity, “everything happening but nothing cohering.”
      
\textbf{Where to look}: Certain psychedelic states before reintegration; dissociative transitions; information overload. 


      
\textbf{Conjecture} (Negative Valence, High Rank, Low Arousal). This combination predicts a state of “expansive despair”—calm hopelessness with full awareness of possibilities, all of which are negative.
...
\textbf{Conjecture} (Rank Exhaustion). Maintaining high $\effrank$ should be metabolically expensive. Prolonged high-rank states should lead to specific fatigue distinct from physical tiredness.
      
\textbf{Where to look}: Post-psychedelic fatigue; meditation retreat collapse (days 3-5); therapist burnout. 


      
\textbf{Conjecture} (Integration Debt). Suppressing integration (compartmentalizing, dissociating) should accumulate “pressure” for reintegration. When defenses fail, the flood should exceed what the original stimulus would warrant.
...
D^{(a)}_{ij} &= |\mathbf{a}_i - \mathbf{a}_j| \quad \text{(info-theoretic affect space)}  D^{(e)}_{ij} &= |\mathbf{e}_i - \mathbf{e}_j| \quad \text{(embedding-predicted affect space)}
...
\emph{Approach 1: Advantage-based.}
...
\emph{Approach 2: Viability-based.} Train a separate probe to predict time-to-death $\tau$ from latent state:
...
\emph{Validation}: $\text{corr}(\Val^{(1)}, \Val^{(2)})$ should be high if both capture the same underlying structure.

      
\textbf{Arousal extraction}. Three approaches:

      
\emph{Approach 1: Belief update magnitude.}
...
\emph{Approach 2: KL divergence.} If the latent is probabilistic (VAE-style):
...
\emph{Approach 3: Prediction error.}
...
\emph{Approach 1: Partition prediction loss.} Train two predictors of $z_{t+1}$:
...
\emph{Approach 2: Attention entropy.} In transformer, attention patterns reveal coupling:
...
\emph{Approach 3: Gradient coupling.} During learning, how do gradients propagate?
...
\emph{Approach 1: Behavioral prediction probe.} Train probe to predict agent’s own future actions from latent state:
...
\emph{Approach 2: Self-other distinction.} In multi-agent setting, probe for which-agent-am-I:
...
\emph{Approach 3: Counterfactual self-simulation.} If agent can answer “what would I do if X?” better than “what would other do if X?”, self-model is present.
...
\textbf{Falsification conditions}:
      ...
      \item \textbf{Identity thesis}: Experience \emph{is} intrinsic cause-effect structure. This is an identity claim, not a correlation.
