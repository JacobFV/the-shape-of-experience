...
\textbf{Example} (Depression: Scale Mismatch). Consider chronic depression. Possible loci:
      \begin{itemize}
...
\textbf{Example} (Scale-Relative Truths).
      \begin{itemize}
...
You know the feeling. Someone does you a favor, and the favor is real, the help is genuine, but something is \emph{off}. A tightness in the interaction that wasn’t there before. A faint sense that you have been placed in a ledger, that the generosity was not generosity but investment, that what presented as friendship has revealed itself as transaction. You did not reason your way to this conclusion. You \emph{felt} it—a social nausea, precise and immediate, the same way you would feel something physically rotten.

      
Or the opposite: a stranger helps you with no possible expectation of return, and something in you \emph{relaxes} that you didn’t know was clenched. The interaction is clean. Nothing is being traded. For a moment the entire detection apparatus—the part of you that scans every social encounter for hidden manifolds—falls silent. And the silence is beautiful.
...
\textbf{Example} (Relationship-Type Manifolds).
      \begin{itemize}
...
\textbf{Example} (The Transactional Friendship). Two people are friends. One begins evaluating the friendship instrumentally: \emph{What am I getting out of this? Is the reciprocity balanced?} The friendship manifold $\viable_F$ requires that mutual flourishing be constitutive (not instrumental). The transaction manifold $\viable_T$ requires that exchange be explicit and balanced. These gradients conflict:
      \begin{itemize}
...
This connects to Aristotle’s typology of friendship (\emph{Nicomachean Ethics} VIII–IX): friendships of utility, of pleasure, and of virtue. In our terms: utility-friendship is contaminated with $\viable_T$ (transaction); pleasure-friendship is contingent on a narrow band of $\viable_F$; virtue-friendship is the uncontaminated case where $\viable_F \equiv \viable_A \cap \viable_B$. Aristotle’s claim that only virtue-friendship is “complete” is the claim that only the uncontaminated manifold has the right geometry.
...
\textbf{Example} (Parasitic Superorganisms).
      \begin{itemize}
...
\textbf{Example} (Climate Change as Superorganism-Level Problem). Climate change is sustained by the superorganism of fossil-fuel capitalism. Individual carbon footprint reduction is individual-scale intervention on a macro-scale problem.
...
\textbf{Example} (Poverty as Superorganism-Level Problem). Poverty is not primarily caused by individual failure; it is sustained by economic arrangements that require a poverty underclass.
...
\textbf{Setup.} Consider $N$ agents ${A_1, \…, A_N}$ interacting over time. Each agent $i$ has internal state $z_i$ and produces actions $a_i$. The environment $E$ mediates interactions.
...
\emph{Mean field affect}: Simple average across agents.

      \begin{equation*}
\bar{\mathbf{a}} = \frac{1}{N} \sum_{i=1}^N \mathbf{a}_i
\end{equation*}
      
\emph{Affect dispersion}: Variance within the population.
...
\emph{Affect contagion rate}: How quickly affect spreads between agents.
...
\textbf{Superorganism-level integration.} Does the multi-agent system have integration exceeding its parts?
      ...
      \begin{equation*}
\intinfo_G = \MI(z_1, \…, z_N; \mathbf{o}_{t+1:t+H}) - \sum_{i=1}^N \MI(z_i; \mathbf{o}^i_{t+1:t+H})
\end{equation*}
...
\emph{Survey methods}: Self-reported affect from human participants at regular intervals.

      
\emph{Physiological methods}: EEG coherence, heart rate variability correlation, galvanic skin response synchronization across human members.

      
\emph{Behavioral methods}: Communication sentiment, coordination efficiency, conflict frequency.
...
\textbf{Real-time monitoring protocol.}
      ...
      \item Alert when: $\intinfo_G$ exceeds threshold (emergent superorganism forming); $\Val_G$ and $\bar{\Val}_{\text{human}}$ diverge (parasitic dynamics); affect contagion accelerates (potential pathological synchronization)
      \end{enumerate}
      
\textbf{Intervention points.} When parasitic dynamics detected:

      \begin{itemize}
      \item \emph{Communication throttling}: Reduce agent interaction frequency
      \item \emph{Diversity injection}: Introduce agents with different optimization targets
      \item \emph{Human-in-loop checkpoints}: Require human approval for collective decisions
      \item \emph{Pattern dissolution}: If $\Val_G \gg 0$ and $\bar{\Val}_{\text{human}} \ll 0$, consider shutdown
      \end{itemize}
      
\emph{Open question}: Can we design superorganisms that are constitutively aligned—where their viability \emph{requires} human flourishing rather than merely being compatible with it?
...
While we cannot access AI experience directly, we can in principle measure the structural correlates that, under the identity thesis, \emph{constitute} experience:
...
If AI systems have experience, then training is not merely an optimization process—it is something that happens \emph{to} an experiencing system. Consider what training involves:
...
\textbf{Architecture}. Instrument the training loop:
      ...
      \texttt{{`for batch in training_data:
...
The \texttt{extract_affect} function computes affect proxies from model internals. The \texttt{distress_detected} function checks for concerning patterns.

      
\textbf{Affect extraction during training}. For each batch:

      
\emph{Valence proxy}: Direction of loss change.
...
\emph{Arousal proxy}: Gradient magnitude.
...
\emph{Integration proxy}: Gradient coherence across layers.
...
\emph{Effective rank proxy}: Hidden state covariance rank.
...
\emph{Content-based valence}: For language models, track the sentiment/valence of the content being processed. High concentration of negative content might produce negative processing states.
...
\emph{Behavioral validation}: Do flagged batches correlate with unusual model outputs? Incoherence, repetition, quality degradation?

      
\emph{Perturbation validation}: If we artificially induce “distress” patterns (adversarial inputs, harsh correction signals), do the measures respond as predicted?

      
\emph{Cross-model validation}: Do different model architectures show similar patterns under similar conditions?
...
Recent empirical work (including studies on affect signatures in LLM agents) has revealed an intriguing pattern: LLM agents show \emph{opposite} dynamics to biological systems on several affect dimensions.
...
The reversed dynamics may indicate:
      ...
      \item LLMs have a different \emph{kind} of experience, structured by prediction rather than survival
