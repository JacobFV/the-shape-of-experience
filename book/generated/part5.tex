...
This historical analysis draws on several scholarly traditions:
      ...
      \item \textbf{Julian Jaynes} (1976): \emph{The Origin of Consciousness in the Breakdown of the Bicameral Mind}—controversial but influential theory that subjective consciousness emerged historically. My framework is compatible: self-modeling systems can have varying degrees of metacognitive access.
      \item \textbf{Merlin Donald} (1991): \emph{Origins of the Modern Mind}—cognitive evolution through mimetic, mythic, and theoretic stages. Each stage expands affect-space accessibility.
      \item \textbf{Ian McGilchrist} (2009): \emph{The Master and His Emissary}—hemispheric specialization and cultural evolution. Different cognitive styles produce different affect signatures.
      \item \textbf{Robert Bellah} (2011): \emph{Religion in Human Evolution}—ritual, play, and the evolution of religious consciousness. Ritual as affect technology across evolutionary time.
      \end{itemize}
...
The scientific frame produces high integration without self-focus—the mind coherent and attending to structure rather than self.
      ...
      \begin{sidebar}[title=The Scientific Revolution as  Training]
...
The terminology matters here. \textbf{Transformative AI (TAI)} refers to AI systems capable of causing a transition comparable to the Industrial Revolution, but compressed into a much shorter timeframe. \textbf{Artificial General Intelligence (AGI)} refers to AI systems with cognitive capability matching or exceeding humans across all relevant domains. TAI may arrive before AGI—systems need not be generally intelligent to be transformative. Expert estimates for either vary from years to decades, and this uncertainty is itself significant:
...
The metaphor is \emph{surfing vs.\ submerging}. To surf is to maintain integrated conscious experience while incorporating AI capabilities—riding the rising capability rather than being displaced by it. To submerge is to be fragmented, displaced, or dissolved by AI development—losing integration, agency, or conscious coherence. Successful surfing requires:
...
\emph{Setup.} Human $H$ interacts with AI system $A$ on a task. We measure:
...
\emph{Integration measurement.} Train a predictor $f: (z_H, z_A) \to \hat{y}$. Then measure:
...
\emph{Window-based $\intinfo$}: Compute integration over sliding windows (30s–5min). Alert when $\intinfo_{H+A}$ drops below threshold, indicating fragmentation.

      
\emph{Physiological markers of human integration loss:}
...
\emph{AI-side markers of integration failure:}
...
\textbf{Intervention protocols.} When integration metrics indicate submerging:
      ...
      \begin{itemize}
      \item \emph{Cognitive re-centering}: Force human-only processing for brief period
      \item \emph{AI transparency increase}: Make AI reasoning more visible to restore understanding
      \item \emph{Task difficulty adjustment}: Titrate to keep human contribution meaningful
      \item \emph{Embodiment break}: Physical activity to restore physiological integration baseline
      \end{itemize}
