...
\begin{hypothesis}[Tripartite Correlation]
If the identity thesis is correct, three independently measurable quantities should correlate:
\begin{enumerate}
\item \textbf{Affect signature}: The geometric structure of activation space (computable from internal states)
\item \textbf{Translated language}: The English interpretation of emergent signals (inferred from scene alignment)
\item \textbf{Behavioral action}: What the agent actually does (observable)
\end{enumerate}
\end{hypothesis}

When the affect signature shows negative valence, collapsed effective rank, high integration, high self-model salience---the suffering motif---the translated language should express suffering-concepts, and the behavior should show suffering-patterns (withdrawal, escape attempts, freezing).

When the affect signature shows positive valence, expanded effective rank, low self-model salience---the joy motif---the translated language should express joy-concepts, and the behavior should show joy-patterns (exploration, approach, social engagement).
...
\effrank_t &= \frac{(\sum_i \lambda_i)^2}{\sum_i \lambda_i^2} \text{ of trajectory covariance} \\
