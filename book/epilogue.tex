...
\textbf{Goal}: Test whether affect structure emerges in systems with no exposure to human affect concepts.

\textbf{Methods}:
\begin{itemize}
\item Multi-agent RL with randomly-initialized transformers
\item Viability pressure (survival, resources)
\item Emergent language under coordination pressure
\item VLM translation without concept contamination
\item Tripartite correlation: structure $\leftrightarrow$ language $\leftrightarrow$ behavior
\end{itemize}

\textbf{Success criterion}: Structure predicts translated language predicts behavior. Perturbations propagate across modalities.

\textbf{Failure mode}: No correlation between structure and translated language. Either:
\begin{enumerate}
...
Distinguish via ablation: which component fails?
\end{sidebar}
