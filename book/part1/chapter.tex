...
\textbf{Selection pressure}. Introduce viability constraints: resource gradients, predator patterns, environmental perturbations. Patterns that model their environment survive longer. Patterns that model themselves survive longer still. The forcing functions from Section 5 apply: partial observability (patterns cannot see beyond local neighborhood), long horizons (resources fluctuate on slow timescales), self-prediction (a pattern's own configuration dominates its future observations).
...
\item \textbf{Multi-channel coupling} (V11.3). Three coupled channels---Structure ($R{=}13$), Metabolism ($R{=}7$), Signaling ($R{=}20$)---with cross-channel coupling matrix and sigmoid gate. Introduces a new measurement: \emph{channel-partition} $\intinfo$ (remove one channel, measure growth impact on remaining channels). Local test: channel $\intinfo \approx 0.01$, spatial $\intinfo \approx 1.0$---channels couple weakly at 3 degrees of freedom. GPU evolutionary trajectory pending.
\end{enumerate}

\textbf{Unexpected}: Mild stress consistently \emph{increases} $\intinfo$ by 60--90\% (Yerkes-Dodson--like inverted-U). Only severe stress causes decomposition. The decomposition threshold shifts upward with heterogeneous chemistry $+$ selection, but the improvement is slow and noisy across 40 cycles---the system remains in decomposition territory.
\end{experiment}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Each substrate enhancement shifts $\Delta\intinfo$ toward the biological pattern (positive). V11.0 naive patterns decompose at $-6.2\%$; V11.2 heterogeneous evolution reaches $-3.8\%$, a +2.1pp shift.}
...
\end{subfigure}
\caption{\textbf{The ladder of substrate complexity and the Yerkes-Dodson effect.} (a)~$\Delta\intinfo$ under severe drought for V11.0--V11.2; (b)~$\intinfo$ response as a function of stress intensity, showing the inverted-U relationship.}
\label{fig:v11-results-a}
\end{figure}

The trajectory from V11.0 through V11.3 reveals a pattern: each step adds internal degrees of freedom for evolution to select on. V11.0--V11.2 operate on a single internal field where chemistry (growth parameters) is the only heritable trait. V11.3 adds multiple coupled fields---channels that must coordinate for growth---introducing a qualitatively new kind of integration (cross-channel) alongside the spatial kind. But three channels may be too few; channel-partition $\intinfo$ is near zero because removing one of three channels is a coarse cut with limited information content.

This motivates V11.4: scaling to $C=64$ continuous channels with fully vectorized physics. At higher dimensionality, evolution has a richer space of coupling configurations to explore, and the spectral structure of channel integration becomes the primary measure. Whether the transition from zombie to biological dynamics requires a \emph{minimum} internal dimensionality---or whether improvement is continuous with $C$---is the central empirical question for this line of investigation.
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Spectral $\intinfo$ trajectory through baseline $\to$ drought $\to$ recovery for $C{=}8$ (integration) and $C{=}32$ (decomposition). The divergence is sharpest during drought onset.}
    \label{fig:v11-phi-timeseries}
\end{subfigure}
\caption{\textbf{Internal dimensionality and stress response.} (a)~$C$-sweep showing the dimensionality dependence of naive stress response; (b)~time-resolved $\intinfo$ dynamics at two channel counts.}
...
The critical lesson remains: the sidebar above predicts that the suffering motif should show $\intinfo$ \emph{high} under threat. But naive CA patterns show the opposite---decomposition, just like LLMs. Neither system has developmental history where integration was selected \emph{for} survival. The forcing functions from the next section are necessary but not sufficient; they must act on a substrate with enough internal degrees of freedom for evolution to discover different integration strategies. Heterogeneous chemistry provides this---different growth parameters create different viability manifold geometries, giving selection something to act \emph{on}.
