...
\textbf{Status}: \emph{Implemented as V12. Three conditions designed:}
\begin{description}
\item[A (Fixed-local attention)] Window size fixed at kernel radius $R$. Tests whether attention expressivity alone (without range expansion) is sufficient. Free-lunch control.
\item[B (Evolvable attention)] Window size $w \in [R, 32]$ is evolvable. The main hypothesis test: does evolution expand perceptual range, and does this enable the biological pattern?
\item[C (FFT convolution)] V11.4 physics as known baseline. Anchors the comparison at $-3\%$ to $-6\%$ $\Phi$ under severe stress.
\end{description}
\emph{Implementation}: Windowed self-attention replaces Step 1 (FFT convolution) of the Lenia scan body. Query-key projections ($W_q, W_k \in \mathbb{R}^{d \times C}$) are shared across space, evolved slowly. Soft distance mask via $\sigma(\beta(w_{\text{soft}}^2 - r^2))$ enables smooth window expansion. Temperature $\tau$ governs attention sharpness. All other physics (growth function, coupling gate, resource dynamics, decay, maintenance) remain identical to V11.4. Curriculum training protocol from V11.7. \\
\emph{Ready for GPU deployment. 9 jobs (3 conditions $\times$ 3 seeds) on A10G.}
\end{experiment}
