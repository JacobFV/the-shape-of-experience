...
What remains open is whether the V10 MARL ablation study \emph{[results pending]} confirms the forcing function hierarchy predicted in the next section---specifically, whether self-prediction and world-model ablations produce the largest reduction in geometric alignment between information-theoretic and embedding-predicted affect spaces.
...
\begin{experiment}[Forcing Function Ablation (V10) --- In Progress]
...
\textbf{Design}: MARL (multi-agent reinforcement learning) with 4 agents navigating a seasonal resource environment. 7 conditions: \texttt{full}, \texttt{no\_partial\_obs}, \texttt{no\_long\_horizon}, \texttt{no\_world\_model}, \texttt{no\_self\_prediction}, \texttt{no\_intrinsic\_motivation}, \texttt{no\_delayed\_rewards}. 3 seeds per condition (21 parallel GPU jobs). Affect measured in the 6D framework; geometric alignment via RSA (representational similarity analysis) between information-theoretic and learned affect spaces. 200k training steps per condition.
\textbf{Prediction}: Self-prediction and world-model ablations will show the largest RSA drop, because these create the strongest coupling pressures (self-referential loops and shared cross-modal representations). Partial observability should have the next-largest effect; intrinsic motivation should matter least for geometric alignment (it primarily aids exploration, not integration).
\textbf{Results}: \emph{[Pending --- 21 jobs training on agi-inc A10Gs, fastest at $\sim$70\% complete.]}
\end{experiment}
