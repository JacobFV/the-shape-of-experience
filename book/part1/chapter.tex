...
\item \textbf{Metabolic maintenance cost} (V11.6). Addresses the autopoietic gap directly: patterns now pay a constant metabolic drain proportional to mass ($\texttt{maintenance\_rate} \times g \times dt$ each step). Without resource income, a pattern decays to zero---drought becomes genuinely lethal, not merely weakening. 30-cycle GPU result ($C{=}64$): \emph{[final numbers pending]}. Preliminary: evolved-metabolic patterns show robustness climbing from $0.968 \to 0.983$ over the first 24 cycles, with Yerkes-Dodson $\intinfo$ increases of $+170$--$200\%$ under mild stress. Fitness function rewards metabolic efficiency (mass maintained per arousal), selecting for calm persistence over frantic survival.
...
\begin{experiment}[Metabolic Lenia (V11.6) --- In Progress]
\textbf{Question}: Does lethal resource dependence change the integration response to stress?
\textbf{Design}: Add a maintenance cost to the HD substrate: each cell pays $\texttt{maintenance\_rate} \times g \times dt$ per step ($\texttt{rate}{=}0.002$). Without resource income, patterns decay to zero---drought is genuinely lethal. Fitness rewards metabolic efficiency (mass maintained per unit arousal).
\textbf{Prediction}: If survival requires active resource-seeking, evolution should select for patterns that \emph{integrate} under moderate scarcity (coordination to find resources) while decomposing only under extreme starvation. This would produce the biological pattern that V11.0--V11.5 failed to achieve.
\textbf{Preliminary} (24/30 cycles complete): Robustness climbing from $0.968 \to 0.983$. Yerkes-Dodson effect persists ($+170$--$200\%$ $\intinfo$ under mild stress). Metabolic fitness selects for calm, efficient patterns over frantic survivors. \emph{Stress test results pending.}
\textbf{Follow-up conditions}: (1) metabolic cost + curriculum training (combining V11.6 and V11.7); (2) metabolic cost + directed motility; (3) metabolic cost + motility + reproduction.
\end{experiment}

What the experiments \emph{have} established is accumulating: (1) Without survival-shaped history, integration under threat does not emerge---this holds across Lenia (V11.0--V11.5), LLMs (V2--V9), and is predicted by the forcing function framework. (2) The \emph{quality} of selection pressure matters: curriculum training (V11.7) produces generalization that fixed-stress training (V11.5) does not. (3) The Yerkes-Dodson pattern (mild stress increasing $\intinfo$ by $60$--$200\%$) is the most robust finding, present across all substrate conditions and channel counts. (4) Stress overfitting provides a genuine parallel to anxiety disorders, and curriculum training provides a parallel to graded exposure therapy. What remains open: whether lethal resource dependence (V11.6) qualitatively changes the dynamics from passive persistence to active self-maintenance, and whether the V10 MARL ablation study \emph{[results pending]} confirms the forcing function hierarchy predicted in the next section.
