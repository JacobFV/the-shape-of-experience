...
\item \textbf{High-dimensional channels} (V11.4). $C{=}64$ continuous channels with fully vectorized physics. Spectral $\intinfo$ via coupling-weighted covariance effective rank. After 12 cycles (30-cycle run in progress): $\intinfo_{\text{base}} \approx 0.024$, $\intinfo_{\text{stress}} \approx 0.063$ ($+165\%$), $n \approx 500$ patterns. The Yerkes-Dodson pattern persists: stress consistently increases mean $\intinfo$ by $+160$--$190\%$ (low-$\intinfo$ patterns die first). Robustness $\approx 0.98$---still below 1.0 (decomposition territory) but selection is slowly improving it.
...
Whether evolution on this richer substrate can discover integration strategies where $\intinfo$ \emph{increases} under threat---the biological pattern---remains the central open question. The current data shows consistent improvement in robustness across the ladder ($-6.2\% \to -3.8\%$ with chemistry, pending $C{=}64$ final results), but the system has not yet crossed into positive territory.
