...
\subsection{Attention as Measurement Selection}\label{sec:attention-measurement}
...
In a CA, this means the following: let $\mathbf{c}_1, \ldots, \mathbf{c}_n$ be cells. A set $\mathcal{B} \subset \{1, \ldots, n\}$ constitutes a \textbf{bounded pattern} if:
\begin{equation}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{$\Delta\intinfo$ under severe drought across substrate conditions. V11.0--V11.2 show progressive improvement; V11.4 (HD, $C{=}64$) further reduces decomposition. V11.5 reveals \emph{stress overfitting}; V11.7 curriculum training substantially reduces it (positive shift on all novel stressors). V11.6 metabolic cost produces efficient passivity, not active foraging.}
...
\begin{experiment}[Attention-Based Substrates]
\textbf{Question}: Does state-dependent interaction topology enable the biological integration pattern that local physics cannot produce?
...
\textbf{Critical prediction}: Under survival pressure, evolution should expand the attention window (increasing perceptual range), and patterns should show the biological pattern---$\intinfo$ \emph{increasing} under moderate stress---because they can dynamically reallocate information flow to maintain core integration. The attention patterns themselves should narrow under stress (focused measurement) and broaden during safety (diffuse exploration).
\textbf{Control for the free-lunch problem}: Start with strictly local attention (window $= R$, matching Lenia's kernel radius). If integration under threat emerges only after evolution expands the window, the biological pattern is an adaptive achievement, not an architectural gift.
\textbf{Status}: \emph{Implemented as V12. Three conditions:}
\begin{description}
...
\textbf{Results} (15 cycles for B, 3 seeds; A and C complete):
\begin{itemize}
...
The V10 MARL ablation study (Experiment~\ref{exp:v10-ablation}) produced a surprise: \emph{all seven conditions show highly significant geometric alignment} ($\rho > 0.21$, $p < 0.0001$), and removing forcing functions does not reduce alignment---if anything, it slightly increases it. The predicted hierarchy was wrong: geometric alignment appears to be a baseline property of multi-agent survival systems, not contingent on any specific forcing function. This strengthens the universality claim but challenges the forcing function theory developed in the next section.
...
The hypothesis is that these pressures increase integration.\label{hyp:forcing-integration} Let $\Phi(\latent)$ be an integration measure over the latent state (to be defined precisely below). Under forcing functions (a)--(f):
\begin{equation}
...
\textbf{Design}: MARL (multi-agent reinforcement learning) with 4 agents navigating a seasonal resource environment. 7 conditions: \texttt{full}, \texttt{no\_partial\_obs}, \texttt{no\_long\_horizon}, \texttt{no\_world\_model}, \texttt{no\_self\_prediction}, \texttt{no\_intrinsic\_motivation}, \texttt{no\_delayed\_rewards}. 3 seeds per condition (21 parallel GPU runs, A10G). Affect measured in the 6D framework; geometric alignment via RSA (representational similarity analysis) with Mantel test ($N{=}500$, 5000 permutations) between information-theoretic and observation-embedding affect spaces. 200k training steps per condition.
...
\begin{tabular}{lcccc}
\toprule
Condition & RSA $\rho$ & $\pm$ std & CKA$_{\text{lin}}$ & CKA$_{\text{rbf}}$ \\
\midrule
\texttt{full} & 0.212 & 0.058 & 0.092 & 0.105 \\
\texttt{no\_partial\_obs} & 0.217 & 0.016 & 0.123 & 0.126 \\
\texttt{no\_long\_horizon} & 0.215 & 0.027 & 0.075 & 0.110 \\
\texttt{no\_world\_model} & 0.227 & 0.005 & 0.091 & 0.103 \\
\texttt{no\_self\_prediction} & 0.240 & 0.022 & 0.100 & 0.120 \\
\texttt{no\_intrinsic\_motivation} & 0.212 & 0.011 & 0.084 & 0.116 \\
\texttt{no\_delayed\_rewards} & 0.254 & 0.051 & 0.147 & 0.146 \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{Partition prediction loss}---the cost of factoring the model:
\begin{equation}
