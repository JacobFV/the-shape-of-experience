...
\subsection{The Affect State Vector}

\begin{definition}[Affect State]
The complete affect state at time $t$ is the six-dimensional vector:
\begin{equation}
\mathbf{a}_t = (\valence_t, \arousal_t, \intinfo_t, \effrank_t, \mathcal{CF}_t, \mathcal{SM}_t)
\end{equation}
Cultural forms, practices, and technologies can be characterized by their characteristic affect signatures---the regions of this space they tend to produce.
\end{definition}
...
A software system to validate the six-dimensional affect framework by comparing predicted signatures with self-report:
...
\item \textbf{Real-time Self-Report Interface}:
  \begin{itemize}
  \item Six sliders (one per dimension) with anchoring descriptions
  \item Continuous sampling at 1 Hz during stimulus
  \item Pre/post discrete ratings for calibration
  \end{itemize}
...
\item \textbf{Prediction Engine}:
  \begin{itemize}
  \item For each stimulus, system predicts $\hat{\mathbf{a}} = (\hat{\valence}, \hat{\arousal}, \hat{\intinfo}, \hat{\effrank}, \hat{\mathcal{CF}}, \hat{\mathcal{SM}})$
  \item Predictions derived from stimulus features (tempo, key, content tags)
  \item Compare $\hat{\mathbf{a}}$ vs.\@ self-report $\mathbf{a}^{\text{reported}}$
  \end{itemize}
\end{enumerate}

\textbf{Experimental Protocol}:
\begin{enumerate}
\item Baseline calibration (neutral stimuli, establish individual anchor points)
\item Target affect induction (present stimulus predicted to induce specific profile)
\item Real-time tracking (continuous self-report during experience)
\item Post-hoc reflection (discrete ratings, qualitative description)
\item Model updating (adjust prediction weights based on error)
\end{enumerate}

\textbf{Validation Metrics}:
\begin{itemize}
\item Per-dimension correlation: $r(\hat{d}_i, d_i^{\text{reported}})$ for each dimension $i$
\item Profile similarity: cosine similarity between predicted and reported vectors
\item Clustering accuracy: do induced affects cluster as predicted in 6D space?
\item Individual differences: do stable person-level parameters emerge?
\end{itemize}

\textbf{Falsification criteria}: If predicted profiles do not predict self-report better than chance, or if clustering structure does not emerge, the six-dimensional basis requires revision.
\end{software}
