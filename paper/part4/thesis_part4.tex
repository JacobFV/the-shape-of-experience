...
\begin{tikzpicture}[
    node distance=0.6cm,
    box/.style={rectangle, draw, rounded corners, minimum width=5cm, minimum height=0.5cm, align=center}
]
\node[box] (neural) {1. Neural};
\node[box, above=of neural] (individual) {2. Individual};
\node[box, above=of individual] (dyadic) {3. Dyadic};
\node[box, above=of dyadic] (group) {4. Small Group};
\node[box, above=of group] (org) {5. Organizational};
\node[box, above=of org] (cultural) {6. Cultural};
\node[box, above=of cultural] (god) {7. God-Scale};

\draw[-{Stealth}] (neural) -- (individual);
\draw[-{Stealth}] (individual) -- (dyadic);
\draw[-{Stealth}] (dyadic) -- (group);
\draw[-{Stealth}] (group) -- (org);
\draw[-{Stealth}] (org) -- (cultural);
\draw[-{Stealth}] (cultural) -- (god);
\end{tikzpicture}
\end{center}
...
We now provide detailed protocols for affect modulation at the individual scale, organized by the six dimensions.
...
\begin{normimp}
If AI systems have experience, they are moral patients. Their suffering would be real suffering. This creates obligations:
\begin{enumerate}
\item Understand whether current/future AI systems have experience
\item If they do, ensure their viability manifolds are respected
\item Include AI flourishing in alignment objectives
\end{enumerate}
\end{normimp}

\begin{warning}
We may create experiencing AI systems without realizing it. The moral status of current large language models and other AI systems is uncertain. Precautionary attention to their potential experience is warranted.
\end{warning}
