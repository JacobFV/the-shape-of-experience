...
export default function Part7() {
  return (
    <>
      ...
      <Sidebar title="Priority 3: The Agency Problem — Partial Progress (V20)">
      ...
      <p><strong>V20 (Protocell Agency)</strong>: Discrete grid world, evolved GRU agents (~3935 params), bounded 5×5 local sensory fields. Actions: move (depletes arrival-patch resources), consume (extracts energy), emit (writes persistent signal traces). Three seeds, 30 cycles × 5000 steps.</p>
      <p><strong>Result</strong>: <M>{"\\rho_{\\text{sync}} \\approx 0.21"}</M> from cycle 0 (70× Lenia's 0.003). Wall broken architecturally. World models present (C<sub>wm</sub> = 0.10–0.15). Self-models emergent in 2/3 seeds (SM<sub>sal</sub> {">"} 1.0). Affect geometry nascent (RSA small but sometimes significant for seed 7).</p>
      <p><strong>Design finding</strong>: Offspring activation bug caused mort=0% throughout — fixed-population soft selection, no bottleneck dynamics. The Bottleneck Furnace (Priority 2) was absent. Expected consequence: affect geometry doesn't fully develop without bottleneck. Next: V20b with fixed offspring activation, 50 cycles. Predicted: RSA crosses 0.2 after first bottleneck event.</p>
      <p><strong>The necessity chain so far</strong>: membrane → world model (✓) → self-model (✓ 2/3 seeds) → affect geometry (◔ nascent). The chain is real. What evolves on top of the architecture is the chain's upper levels.</p>
      </Sidebar>
      ...
      <li><strong>What remains</strong>: Bridge to human neuroscience (Priority 1). V20b with bottleneck dynamics to complete the necessity chain (affect geometry following self-model, Priority 3). Scale social integration toward superorganism detection (Priority 4). Track AI affect across training regimes (Priority 5).</li>
      </ol>
