...
export default function Part6() {
  return (
    <>
      ...
      <p>This is not merely a story about ideas accumulating; it is a story about expanding the dimensionality of the problem spaces that human collectives can navigate together. Animist cultures solved problems in the three-dimensional inhabited world, coordinating activity around the affordances of immediate landscape and the rhythms of biological time. The Axial civilizations opened a new dimension: self-model space—the space of possible ways to hold and relate to one's own experience—making metacognitive navigation available as a cultural inheritance rather than an individual discovery. The scientific revolution added abstract model space: the capacity to coordinate around representations that work at scales from subatomic to cosmological, severed from any particular embodied perspective. The digital transition is adding something else: the distribution of cognition itself across substrates and continents, making it possible to coordinate inference and memory at population scale. Each era's consciousness technology does not just expand what can be known or felt; it expands the dimensionality of what can be jointly navigated—the problem space over which collective human integration can operate.</p>
      ...
      <p>There is a shadow in this picture. Each expansion of collective coordination comes with pressure to simplify how individual interiority gets modeled. Institutions that span millions cannot afford participatory perception of each member; they optimize for behavioral observables. Digital infrastructure accelerates interaction at the cost of depth. The <M>{"\\iota"}</M> framework names what this produces: not individual mind-blindness but <em>institutional</em> mind-blindness—systems structurally optimized for high-<M>{"\\iota"}</M> observation of persons, producing environments in which being perceived as a behavioral variable rather than as a navigating subject is the default experience of modern life. The modern meaning crisis is partly this: an architecture of collective perception from which the full interiority of those inside it is structurally excluded. The corrective is not merely lower-<M>{"\\iota"}</M> individuals but lower-<M>{"\\iota"}</M> institutions—social structures that model the high-dimensional problem spaces their members are navigating, not just the behavioral outputs of those navigation attempts.</p>
