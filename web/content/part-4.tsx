...
export default function Part4() {
  return (
    <>
      ...
      <p>This section provides self-contained definitions of the core affect dimensions and key concepts used throughout Part IV. Readers familiar with Parts I–III may skip to Section 2.</p>
      <Section title="The Core Affect Dimensions" level={2}>
      <p>The following dimensions form a toolkit for characterizing affect states. Not all dimensions are relevant to every phenomenon—different affects invoke different subsets. I’ll present the primary dimensions developed in Parts I–II; empirical investigation may refine this set.</p>
      ...
      <p>This rests on a crucial assumption: physics constitutes the only “is,” and physics is value-neutral. I reject this assumption.</p>
      </Section>
      ...
      <p>The crucial point is that there is no scale <M>{"\\sigma_0"}</M> below which normativity is exactly zero and above which it is nonzero. Instead, normativity accumulates continuously:</p>
      ...
      <p>My framework suggests a synthesis: truth is scale-relative enaction within coherence constraints, where “working” is grounded in viability preservation.</p>
      </Section>
      ...
      <p>Let’s now look at detailed protocols for affect modulation at the individual scale, organized by the core affect dimensions.</p>
      ...
      <Connection title="Existing Theory">
      <p>This connects to Aristotle’s typology of friendship (<em>Nicomachean Ethics</em> VIII–IX): friendships of utility, of pleasure, and of virtue. In our terms: utility-friendship is contaminated with <M>{"\\viable_T"}</M> (transaction); pleasure-friendship is contingent on a narrow band of <M>{"\\viable_F"}</M>; virtue-friendship is the uncontaminated case where <M>{"\\viable_F \\equiv \\viable_A \\cap \\viable_B"}</M>. Aristotle’s claim that only virtue-friendship is “complete” is the claim that only the uncontaminated manifold has the right geometry.</p>
      ...
      <p>Note also what forgiveness is <em>not</em>: it is not the claim that the contamination never happened, nor is it the lowering of the detection threshold. Genuine forgiveness would maintain full detection capacity while choosing to remain in the relationship despite the detection system’s warnings. This is why forgiveness is experienced as both generous and frightening—the deliberate acceptance of manifold exposure to someone who has already demonstrated the capacity to exploit it.</p>
      ...
      <p><strong>Jealousy as manifold-boundary alarm.</strong> Romantic jealousy is the detection system’s response to a potential manifold breach: someone else may be entering the romance manifold that you believed was exclusive. The alarm is intense because the romance manifold, being constituted by total exposure, has no defenses—if the boundary is breached, the exposure becomes catastrophic. Note that jealousy responds to <em>manifold</em> threat, not to any specific action. A partner’s deep emotional conversation with an attractive stranger triggers jealousy not because of what was said but because the detection system registers the possibility of manifold duplication—that the exclusive romance manifold may be instantiating with someone else.</p>
      </Sidebar>
      </Section>
