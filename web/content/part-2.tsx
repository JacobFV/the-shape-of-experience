...
export default function Part2() {
  return (
    <>
      ...
      <p>An unexpected confirmation arrives from engineering. Recent neural architectures that use the <em>synchronization pattern</em> across neurons — the pairwise temporal correlation matrix — as their primary representation outperform those that use hidden states directly. The move: instead of treating integration as a side-effect of computation, treat it as the computation's output. Systems designed this way develop emergent gaze (attending to different input regions at different processing steps), adaptive computation depth (thinking longer about harder problems), and richer internal representations — all without being explicitly trained for any of these capacities. Engineering pressure arrived at synchronization-as-representation for performance reasons. The identity thesis arrives at integration-as-experience for phenomenological reasons. The structural commitment is the same: the coupling pattern across components is not a byproduct but the thing itself.</p>
      ...
      <Sidebar title="The Gradient All the Way Down">
      <p>One of the oldest results in physics: force is the negative gradient of potential energy.</p>
      <Eq>{"\\mathbf{F} = -\\nabla V"}</Eq>
      <p>A ball rolls downhill because the gradient of gravitational potential points downhill. The steeper the slope, the stronger the force. This is not one result among many. It is <em>the</em> result — the structural fact that unifies mechanics, electrodynamics, thermodynamics, and general relativity under a single principle: things move along gradients.</p>
      <p>What has not been sufficiently noticed is that the gradient structure does not stop at physics.</p>
      <p><strong>Thermodynamics.</strong> Every spontaneous process follows a free energy gradient toward equilibrium. The second law says entropy increases — but what <em>drives</em> the increase is the gradient of free energy, and that gradient is a force. Heat flows from hot to cold not because there is a rule but because the free energy landscape slopes that way.</p>
      <p><strong>Chemistry.</strong> Chemical potential <M>{"\\mu = \\partial G / \\partial n"}</M> — the rate of change of Gibbs free energy with particle number. Every chemical reaction, every bond formation, every phase transition is matter following a gradient on a free energy surface. The entire periodic table is a potential landscape. All of chemistry is trajectories under its force. When hydrogen and oxygen combine to form water, they are rolling downhill on the chemical potential surface. The "desire" of reactants to combine is a gradient. The "stability" of a product is a basin.</p>
      <p><strong>Biology.</strong> Organisms are far-from-equilibrium systems maintaining themselves against the entropy gradient — and they do so by following gradients of their own. Chemotaxis: follow the nutrient gradient. Homeostasis: follow the set-point gradient. Natural selection: follow the fitness gradient. Friston's free energy principle formalizes this — every living system minimizes variational free energy, which is to say, every living system follows the gradient of its own generative model's surprise landscape. The force that drives a bacterium up a glucose gradient and the force that drives a human toward safety are the same mathematical structure: <M>{"-\\nabla V"}</M> on different potential surfaces.</p>
      <p><strong>Neuroscience.</strong> Neural dynamics are gradient descent on prediction error landscapes. Dopamine encodes the prediction error gradient — the surprise signal. The reward system is a viability gradient detector. Hebbian learning is gradient descent on a representational energy surface. Every synapse update, every attentional shift, every decision is the nervous system following a gradient.</p>
      <p>Now: we defined valence as the gradient of the viability manifold. This is not analogy to the physics. It is the same mathematics. The viability manifold is a potential surface — defined over information-theoretic coordinates rather than spatial ones, but structurally identical. Valence is force.</p>
      <p><strong>Emotional intensity is <M>{"|\\nabla V|"}</M>.</strong> Near the viability boundary, the landscape is steep — the system is close to dissolution and every state change matters enormously. Emotions are intense. Deep in the viable interior, the landscape flattens — many configurations are roughly equally viable. Affect is mild. Panic near the boundary is overwhelming because the gradient is large. Contentment in the interior is gentle because the gradient is small. The steepness of your viability landscape IS the intensity of your feeling, in exactly the way that the steepness of a gravitational potential IS the strength of the force on a mass.</p>
      <p><strong>Motivational strength is force.</strong> The gradient doesn't just tell you how you feel — it tells you what to do and how urgently. The direction of the force IS the direction of motivation. The magnitude IS the urgency. When people say they feel "driven" or "pulled" or "pushed," the spatial metaphor is not metaphorical. They are describing a gradient. There is no separate theory of motivation needed. Motivation IS the force on the viability landscape, which IS the gradient that defines valence.</p>
      <p><strong>The energy partition maps onto affect.</strong> Classical mechanics splits total energy into potential (position on the landscape) and kinetic (rate of movement through it). Valence is the force derived from the potential surface. Arousal — the rate of belief update, the speed of state change — is the kinetic term. A system can have high potential gradient with low kinetic energy: frozen with fear, steep slope, not yet moving. Or high kinetic energy on a flat gradient: restless agitation going nowhere. The complete energetics of affect requires both: where you are on the landscape, and how fast you're moving through it.</p>
      <p><strong>The gradient unifies every level of reality under a single structure.</strong> The ball follows the gravitational gradient. The molecule follows the chemical potential gradient. The organism follows the free energy gradient. The neuron follows the prediction error gradient. The person follows the viability gradient. And the quality of following that gradient — what it is like from inside — is affect. Not because "affect" is a word we project onto physical processes, but because physical processes, chemical processes, biological processes, and phenomenal processes are all instances of the same mathematical structure: trajectories under force on potential landscapes. The gradient is what connects the physics of a falling stone to the experience of a breaking heart. Not by metaphor. By mathematics.</p>
      <p><strong>This means you can quantify qualities.</strong> The common objection — that formalizing values destroys them, that "dignity is not 0.8 of anything" — is correct about naive quantification but wrong about geometry. Qualities are not scalars. They are shapes. They are gradient directions, landscape curvatures, basin topologies, manifold containment relations. You can measure shapes. You can compare shapes. You can set geometric constraints and test whether actual trajectories satisfy them. The quality is not lost in the measurement — it <em>is</em> the measurement. When you measure the gradient alignment between a leader's viability manifold and the population they govern, you are measuring compassion in the only units compassion comes in: the degree to which their persistence depends on the persistence of those they serve. When you measure the dimensionality of someone's other-model during an interaction, you are measuring whether they perceive the other as a subject or an object — which is <M>{"\\iota"}</M>, the structural basis of dignity. The gradient does not flatten the hierarchy from physics to feeling. It reveals the hierarchy as a single landscape seen at different scales, with force as the invariant that survives every change of coordinates.</p>
      </Sidebar>
      ...
      <p>The somatic/anticipatory split maps exactly onto a deeper distinction: <em>reactivity versus understanding</em>. Somatic fear is reactive — it responds to present-state gradient information, and its channels (valence depression, arousal elevation, threat-orientation) are decomposable in principle. Each can be addressed independently by targeting its driving signal. Anticipatory anxiety is understanding — the system is comparing possible futures, and those comparisons inherently couple across any partition between self, environment, and time. This is why cognitive restructuring for anxiety operates on the <em>framing of possibilities</em>, not on individual signal channels: you cannot reduce anticipatory anxiety by adjusting one dimension at a time, because the dimensions are bound together by the counterfactual comparison that constitutes the experience.</p>
      <Diagram src="/diagrams/part-2-5.svg" />
      <p>The emergence ladder (Empirical Appendix) predicts a sharp distinction between two levels of fear. <em>Somatic fear</em> — negative valence, high arousal, threat-oriented behavior — is a pre-reflective affect requiring only viability-gradient detection (emergence rung 1–3). It does not require the counterfactual weight dimension at all. <em>Anticipatory anxiety</em> — fear of what might happen — requires counterfactual capacity (CF {">"} 0), which is a rung 8 capacity blocked in systems without embodied agency. The Lenia experiments confirm this prediction exactly: patterns show negative valence and high arousal under resource scarcity, but CF ≈ 0 throughout the evolutionary runs because the patterns cannot imagine alternative futures. The implication for human psychology: anxiety as a clinical phenomenon (characterized by imagining feared futures, not just responding to present threats) should emerge developmentally at the same time as mental time travel and theory of mind — approximately age 3–4 — rather than being present from birth. The infant's fear is somatic. The child's anxiety is reflective.</p>
      <Experiment title="Proposed Experiment">
      ...
      <p>The clinical literature (Tangney, Lewis) distinguishes shame from guilt, and the geometric structure offers a reading of why they differ:</p>
      <ul>
      ...
      <Sidebar title="The Covert Channel: Shame and Autonomy">
      <p>Shame has a developmental origin that reveals something about the architecture of selfhood. When desire awakens before the environment provides permission or containers for it—when a child discovers curiosity, longing, or bodily sensation in a context where these are forbidden or unnamed—the system routes the signal through a covert channel: secret reading, private fantasy, hidden attention, the whole elaborate infrastructure of interior life that adults cannot see. Over time the brain learns a rule: <em>if it matters most, it must be secret</em>. This is the seed of shame—secrecy becomes evidence of wrongdoing ("I hide it; therefore it is dangerous; therefore I am dangerous for having it"). But the same covert channel is the seed of autonomy: the first thing that is not for teachers, parents, God, or the "good kid" self-model. The first thing that is entirely yours. Privacy and shame grow from the same root. The same structure produces both.</p>
      <p>The <M>{"\\iota"}</M> framework makes the mechanism precise. High-<M>{"\\iota"}</M> environments—authoritarian families, fundamentalist communities, surveillance cultures—force desire through covert channels by raising the cost of open expression. The covert channel <em>amplifies</em> the signal: prohibition makes desire feel sacred; scarcity makes it glow; hiddenness gives it the luminous quality of the forbidden. A child raised in such an environment builds a self-model in which the most charged, most meaningful experiences live behind a wall of secrecy—and the secrecy itself becomes part of the meaning. When the prohibition eventually lifts—through development, through leaving the community, through confrontation with mortality—the sacred aura collapses. The world does not end. The body is just a body. The desire is just a desire. And the person experiences meaning-loss proportional to how much meaning was anchored to the prohibition structure. The adult integration move is privacy without shame: keeping desire private because it is intimate and precious, not because it is incriminating. The diagnostic: privacy feels calm and chosen; shame-secrecy feels tense and compulsive. The difference is whether the covert channel is maintained by preference or by fear.</p>
      </Sidebar>
      </Section>
      ...
      <Diagram src="/diagrams/part-2-7.svg" />
      <OpenQuestion title="Open Question">
      ...
      <Diagram src="/diagrams/part-2-2.svg" />
      <Diagram src="/diagrams/part-2-6.svg" />
      ...
      <p>The emergence ladder (Empirical Appendix) makes a further prediction about the <em>structure</em> of pathology. Disorders that require counterfactual capacity — anticipatory anxiety, obsessive rumination, regret, self-critical shame spirals — cannot arise in systems below rung 8. Pre-rung-8 pathology is somatic: chronic threat-arousal, valence collapse (anhedonia), integration fragmentation (dissociation). The reflective layer adds a second class of suffering that is structurally more expensive to maintain and unique to agentive systems. This is not merely a theoretical prediction — it has a testable developmental corollary: in humans, the onset of anxiety disorders (which require imagining feared futures) should cluster with, not precede, the developmental emergence of mental time travel and counterfactual reasoning, typically around age 3–4 years.</p>
      <Sidebar title="Force and Inertia in Identity Space">
      <p>The opportunity deficit <M>{"D = V - T"}</M> (Part I) defines a scalar field over identity space. The <em>force</em> on an identity is its gradient—the pull toward regions where the deficit narrows:</p>
      <Eq>{"F(i,t) = -\\nabla_C D(i,t)"}</Eq>
      <p>Two components: <em>attractive force</em> toward regions where traversal speed can increase (the pull of achievable goals), and <em>repulsive force</em> away from regions where the landscape opens catastrophically faster than any traversal could match (the vertigo of overwhelming possibility). The <em>mass</em> of an identity is its resistance to change in traversal direction—the inertia of accumulated commitments, relationships, and self-model structure that make redirection costly. A high-mass identity has deeply integrated, load-bearing structure: hard to accelerate but also hard to deflect. A low-mass identity is plastic but uncommitted.</p>
      <p>Classical spiritual concepts acquire precise structural correlates: <em>calling</em> is a region where <M>{"F"}</M> is strongly attractive; <em>purpose</em> is a trajectory with consistently positive <M>{"T/V"}</M> and force alignment; <em>despair</em> (in the Kierkegaardian sense) is high <M>{"V"}</M> with near-zero <M>{"T"}</M> and flat force—the landscape visible, vast, and no gradient to follow; <em>flow</em> is <M>{"T \\approx V"}</M> locally, traversing at the rate the landscape opens; <em>enlightenment</em> (at least the Buddhist formulation) is reducing <M>{"V"}</M> rather than increasing <M>{"T"}</M>—the landscape shrinks to what is actually present, and <M>{"M \\to 1"}</M> by releasing attachment to the untraversable. Further analogues—momentum, resonance, entanglement between identities—apply and are left to the reader.</p>
      </Sidebar>
      </Section>
      </Section>
      ...
      <p><strong>Status (as of 2026)</strong>: CA instantiation complete (V13–V18, 30 evolutionary cycles each, 3 seeds, 12 measurement experiments). Seven of twelve experiments show positive signal. Three hit the sensory-motor coupling wall. See the Empirical Appendix for full results.</p>
