...
export default function Part1() {
  return (
    <>
      ...
      <p>V22–V24 provided that gradient — within-lifetime prediction learning via SGD through the internal ticks — and confirmed both halves of the hypothesis. Learning works (100–15,000× prediction improvement per lifetime), but prediction accuracy, target breadth, and time horizon are all individually insufficient to create integration. A deeper analysis revealed why: the 16-dimensional hidden state had collapsed to an effective rank of 1–3 across all seeds, encoding <em>only energy level</em> (decoding <M>{"R^2 \\approx 1.0"}</M> for energy, <M>{"R^2 \\approx 0"}</M> for position and resources). On a uniform 128×128 grid, the optimal policy is "track energy, eat when you can" — one dimension suffices. Prediction heads were optimizing the surface of a degenerate representation. The environment, not the agent, was the bottleneck: when survival demands only reactivity, evolution strips state to the minimum, and no prediction architecture can force integration over a one-dimensional substrate. Understanding requires worlds that <em>demand</em> understanding.</p>
