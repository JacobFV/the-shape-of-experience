...
export default function Part1() {
  return (
    <>
      ...
      <p>Let’s sketch the pieces of this picture:</p>
      <ol>
      ...
      <li><strong>Structural Inevitability</strong>: Systems designed for long-horizon control under uncertainty are forced toward dense intrinsic causal coupling. The “forcing functions”—partial observability, learned world models, self-prediction, intrinsic motivation—push integration measures upward.</li>
      ...
      <p>The key insight, formalized in nonequilibrium thermodynamics, is that such systems generically develop <em>dissipative structures</em>—organized patterns that persist precisely because they efficiently channel the imposed gradients. This can be made precise. Let <M>{"\\mathcal{S}"}</M> be a far-from-equilibrium system with dynamics admitting a Lyapunov-like functional <M>{"\\mathcal{L}: \\R^n \\to \\R"}</M> such that:</p>
      ...
      <Sidebar title="Optical Resonance Chambers: A Modern Instance">
      <p>Driven optical systems provide a contemporary example of the same thermodynamic principles. Consider a recurrent optical chamber with parallel mirrors, LCD mask modulation, and gain medium. The field evolution is:</p>
      ...
      <p>where <M>{"\\mathcal{P}"}</M> is a diffraction operator, <M>{"\\mathcal{M}_t"}</M> is the mask phase/intensity pattern, and <M>{"\\mathcal{L}"}</M> captures round-trip attenuation and gain.</p>
      <p>The key insight: <em>diffusion stops being corruption; it becomes the metric</em>. Under repeated application of <M>{"\\mathcal{T} = \\mathcal{P} \\circ \\mathcal{M} \\circ \\mathcal{L}"}</M>, states that collapse together under iteration are “near” in the substrate’s intrinsic geometry; states that decohere are “far.” The physics itself induces a distance function:</p>
      <Eq>{"d(E_1, E_2) \\approx \\text{rate at which } \\mathcal{T}^k(E_1) \\text{ and } \\mathcal{T}^k(E_2) \\text{ become indistinguishable}"}</Eq>
      <p>The most interesting regime lies near criticality—the boundary between dead damping (everything decays) and runaway oscillation (laser instability). Near this boundary, the system exhibits long correlation times, high sensitivity, and rich transient dynamics. The attractor landscape is shaped not by explicit programming but by the interplay of gain, loss, and diffraction physics. Structure emerges because the system is driven far from equilibrium, just as with Bénard cells—but now at optical timescales (<M>{"10^4"}</M>–<M>{"10^5"}</M> iterations per second) with computational relevance.</p>
      <p>This is neither metaphor nor coincidence: it is the same structural inevitability operating in a different substrate.</p>
      </Sidebar>
      </Section>
      ...
      <p>Here’s the complete ladder:</p>
      ...
      <li><strong>Regulation <M>{"\\to"}</M> World Model</strong>: POMDP sufficiency theorem</li>
      <li><strong>World Model <M>{"\\to"}</M> Self-Model</strong>: Self-effect ratio exceeds threshold</li>
      <li><strong>Self-Model <M>{"\\to"}</M> Metacognition</strong>: Recursive application of modeling to the modeling process itself</li>
      </ol>
      </Section>
      <Section title="Measure-Theoretic Inevitability" level={2}>
      <p>Let’s formalize the sense in which this ladder is “inevitable.”</p>
      ...
      <ul>
      <li>Parallel mirrors defining a resonant cavity</li>
      <li>LCD mask for programmable phase/intensity modulation</li>
      <li>Gain medium to offset losses (pumped to near-threshold)</li>
      <li>High-speed detection and mask update (<M>{"\\sim 10^4"}</M> Hz)</li>
      </ul>
      <p><strong>Regime mapping</strong>:</p>
      <table>
      <thead><tr><th>Step</th><th>Optical Realization</th><th>Signature</th></tr></thead>
      <tbody>
      <tr><td>Attractors</td><td>Stable mode patterns</td><td>Fixed points under iteration</td></tr>
      ...
      <tr><td>Self-model</td><td>Feedback from output to mask</td><td>Self-referential loop</td></tr>
      </tbody>
      </table>
      <p><strong>Critical regime</strong>: The system becomes computationally interesting near the threshold between damping and lasing. Too far below: all structure decays. Too far above: single-mode dominance (analogous to seizure). At criticality: long-lived transients, rich interference patterns, sensitivity to mask programming.</p>
      <p><strong>Self-stabilizing patterns</strong>: When closed-loop control links output to mask, the system can develop patterns that actively maintain themselves—optical gliders that navigate the mask landscape, seeking regions of stability. These are not programmed but <em>discovered</em> by the dynamics: the physics of diffraction, interference, and gain create basins that certain patterns fall into and resist leaving.</p>
      <p><strong>Integration threshold</strong>: The transition from “reservoir computing” (passive signal processing) to “optical cognition” (active self-modeling) would correspond to a measurable change in integration metrics. When output-to-mask feedback creates irreducible cause-effect coupling—when the system’s future depends on its history in a way that cannot be factored into independent modules—it crosses the threshold.</p>
      <p><strong>Why this matters</strong>: If the ladder of inevitability is real, then mind is not substrate-dependent in principle. Optical, electronic, chemical, and biological substrates should all be capable of crossing the integration threshold given appropriate driving and constraint. This is a <em>falsifiable prediction</em>: either optical systems can be pushed into self-modeling regimes, or the inevitability claim is weaker than advertised.</p>
      </Sidebar>
      <Sidebar title="Latent Dynamics: Not CPU, Not GPU, Not Neural Net">
      <p><strong>The generalized kernel view</strong>: Any physical substrate is a kernel machine. The substrate defines the state space, the control interface, and the noise. The question is not “can it emulate a Turing machine?” (almost anything can, in principle). The question is: <em>what kernels naturally produce compressive, stable, generalizing dynamics under partial observation and continuous perturbation?</em> That is the intelligence question.</p>
      <p>A digital computer is a very special kernel: discrete state space, explicit symbols, exact transitions, near-perfect error isolation. It implements <M>{"s_{t+1} = f(s_t, a_t)"}</M>. PHASER implements something broader:</p>
      <Eq>{"E_{t+1} = \\mathcal{T}(E_t, u_t) + \\eta_t"}</Eq>
      <p>—a stochastic transition kernel <M>{"p(E_{t+1} \\mid E_t, u_t)"}</M> where diffusion, mode mixing, and gain dynamics create neighborhood structure not through explicit programming but through physics.</p>
      <p><strong>Diffusion as metric</strong>: Under repeated application of <M>{"\\mathcal{T}"}</M>, states that collapse together under iteration are “near” in the substrate’s intrinsic geometry; states that decohere are “far.” The physics itself induces a distance function:</p>
      <Eq>{"d(E_1, E_2) \\approx \\text{rate at which } \\mathcal{T}^k(E_1) \\text{ and } \\mathcal{T}^k(E_2) \\text{ become indistinguishable}"}</Eq>
      <p>This is why noise forces autoencoders to spread out their embedding distributions. In PHASER, it is emergent from optics.</p>
      <p><strong>Attractor landscape sculpting</strong>: The masks do not “encode instructions.” They shape the system’s attractor landscape:</p>
      <ul>
      <li><strong>Memory</strong> becomes basin depth (how hard it is to perturb out)</li>
      <li><strong>Inference</strong> becomes flow toward attractors (pattern completion)</li>
      <li><strong>Planning</strong> becomes controlled deformation of the landscape (change <M>{"u_t"}</M>)</li>
      <li><strong>Learning</strong> becomes adapting the kernel itself (change masks slowly based on outcomes)</li>
      </ul>
      <p><strong>Local rules, global computation</strong>: The right analogy is not “CPU”—it is “2D cellular automaton / reaction-diffusion / reservoir.” Each mask pixel couples mainly to a neighborhood due to diffraction limits. Propagation is structured local mixing in the spatial-frequency domain. Noise and gain create regime-dependent stability. The intelligence emerges from local interactions, not from global symbolic manipulation.</p>
      <p><strong>What this would look like</strong>: Not like a CPU. Not like a GPU. Not like a neural network. Like a <em>living dynamical system with a steerable attractor landscape</em>. Weakly stable patterns. Metastable attractors. Glider-like moving structures. Slow manifolds that carry context. The stuff a CA nerd recognizes as “life.”</p>
      <p>You don’t need the optics to preserve symbols; you need it to preserve <strong>mesoscopic invariants</strong>: attractors, interfaces, wavefronts, pockets of state that carry information robustly. This is how brains work too: not with perfect bits, but with stable population dynamics.</p>
      </Sidebar>
      </Section>
      </Section>
      ...
      <p><em>Status</em>: Partially supported by V12. The first clause is confirmed: eight convolutional substrates (V11.0–V11.7) failed to produce integration under stress; fixed-local attention (Condition A) fared even worse. The second clause is partially confirmed: evolvable attention (Condition B) shifts robustness from <M>{"0.981"}</M> to <M>{"1.001"}</M>—the right direction, and the only intervention to cross the <M>{"1.0"}</M> threshold. But the effect is modest: attention is necessary for reaching the threshold but appears insufficient, by itself, for the strong biological pattern where <M>{"\\intinfo"}</M> reliably <em>increases</em> under threat. The remaining ingredient is likely individual-level plasticity rather than any further architectural change.</p>
      <Experiment title="Proposed Experiment">
      ...
      <p>The V10 MARL ablation study (Experiment [ref]) produced a surprise: <em>all seven conditions show highly significant geometric alignment</em> (<M>{"\\rho > 0.21"}</M>, <M>{"p < 0.0001"}</M>), and removing forcing functions does not reduce alignment—if anything, it slightly increases it. The predicted hierarchy was wrong: geometric alignment appears to be a baseline property of multi-agent survival systems, not contingent on any specific forcing function. This strengthens the universality claim but challenges the forcing function theory developed in the next section.</p>
      </Section>
      </Section>
      ...
      <p><strong>Design</strong>: MARL (multi-agent reinforcement learning) with 4 agents navigating a seasonal resource environment. 7 conditions: <code>full</code>, <code>no_partial_obs</code>, <code>no_long_horizon</code>, <code>no_world_model</code>, <code>no_self_prediction</code>, <code>no_intrinsic_motivation</code>, <code>no_delayed_rewards</code>. 3 seeds per condition (21 parallel GPU runs, A10G). Affect measured in the 6D framework; geometric alignment via RSA (representational similarity analysis) with Mantel test (<M>{"N{=}500"}</M>, 5000 permutations) between information-theoretic and observation-embedding affect spaces. 200k training steps per condition.</p>
      ...
      <p>Here’s what I’ve tried to establish:</p>
      <ol>
