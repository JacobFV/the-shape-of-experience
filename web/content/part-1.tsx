...
export default function Part1() {
  return (
    <>
      ...
      <p>in bits per unit time. The <em>meaning ratio</em> <M>{"M = T/V"}</M> is the fraction of perceived possibility being actualized. When <M>{"M \\to 1"}</M>, the identity keeps pace with what it can see. When <M>{"M \\to 0"}</M>, vast landscape, minimal traversal—the Frankl condition. The <em>meaning deficit</em> <M>{"D = V - T"}</M> is the gap in bits between seeing and doing. This deficit scales with cognitive capacity: the hunger for meaning is not new, but as symbolic capacity expands, the mouth gets bigger. The landscape grows at least exponentially with the mind's effective rank <M>{"\\effrank"}</M> (volume in high-dimensional spaces), while traversal speed grows at most linearly. The ratio problem is structural, not motivational—and it worsens as intelligence scales.</p>
      </Sidebar>
      </Section>
