...
export default function Part1() {
  return (
    <>
      ...
      <p>V13–V18 extended this program with six additional substrate variants and twelve measurement experiments, sharpening the conclusion considerably. The geometry is confirmed more strongly: affect dimensions develop over evolution (Exp 7), the participatory default is universal and selectable (Exp 8), and collective coupling amplifies individual integration (Exp 9). But the dynamics wall was located precisely: at what Part VII calls rung 8 of the emergence ladder — the point where counterfactual sensitivity and self-modeling become operational. Substrate engineering (memory channels, attention, signaling, insulation fields) could not cross this rung. All variants shared the same limitation: <M>{"\\rho_{\\text{sync}} \\approx 0.003"}</M>. Patterns were passengers, not causes.</p>
      <p>V20 crossed it. Protocell agents — evolved GRU networks with bounded local sensory fields and discrete actions — achieve <M>{"\\rho_{\\text{sync}} \\approx 0.21"}</M> from initialization, before any evolutionary selection. World models developed over evolution. Self-models emerged in 2/3 seeds: agents encoded their own position and energy in their hidden state more accurately than they encoded the environment — the minimal form of privileged self-knowledge. Affect geometry appeared nascent, consistent with needing additional selection pressure to fully develop. The necessity chain — membrane, free-energy gradient, world model, self-model, affect geometry — holds through self-model emergence in an uncontaminated substrate. Not "biography" as a vague metaphor, then, but "action as cause" as a testable architectural requirement. The experiments now specify both sides of that threshold.</p>
