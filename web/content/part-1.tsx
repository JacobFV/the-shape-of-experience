...
import { Align, Connection, Diagram, Empirical, Eq, Experiment, Figure, Historical, Illustration, Logos, M, NormativeImplication, OpenQuestion, Proof, Section, Sidebar, TodoEmpirical } from '@/components/content';
...
export default function Part1() {
  return (
    <>
      ...
      <Sidebar title="The Spectrum of Persuadability">
      <p>Michael Levin observes that our relationship to any system — how we can meaningfully intervene in it — follows a spectrum: from hardware modification (direct physical manipulation), to control-theoretic forcing (feedback and setpoints), to behavioral shaping (training, reward), to linguistic persuasion and negotiation, to psychoanalysis, friendship, and love. The higher on this spectrum you can engage a system, the more you are treating its interiority as real — modeling the goals it holds, the meanings it assigns, the self-representations it maintains. The lower on the spectrum, the more you are treating it as a mechanism whose behavior is controlled at the level of its substrate.</p>
      <p>This is the <M>{"\\iota"}</M> parameter made concrete. A system perceived at <M>{"\\iota = 1"}</M> — full mechanistic reduction — admits only hardware modification. A system perceived at <M>{"\\iota \\approx 0"}</M> — full participatory attention — is reachable by love. The spectrum of persuadability is the practical face of the inhibition coefficient: what level of tool is appropriate depends entirely on which ι you bring to the encounter. And that choice is itself a statement about what you take the system to be.</p>
      </Sidebar>
      ...
      <li><strong>Structural Inevitability</strong> (hypothesis): Systems designed for long-horizon control under uncertainty are predicted to develop dense intrinsic causal coupling. The candidate "forcing functions"—partial observability, learned world models, self-prediction, intrinsic motivation—should push integration measures upward. This is the least secure of the three inevitability claims; experimental tests have so far failed to confirm it in the expected form (Empirical Appendix).</li>
      ...
      <p>This trajectory-selection mechanism operates at the population level too. In evolutionary experiments (V31), different seeds follow different trajectories through the same dynamical landscape — not because their initial conditions differ (all start identically) but because the drought-recovery measurement distribution differs: which agents survive each bottleneck selects which evolutionary path the population follows. The correlation between post-drought recovery and mean integration across seeds is <M>{"r = 0.997"}</M>. The measurement distribution — which perturbations are survived rather than which are attended to — selects the trajectory. The equation is the same; the scale is different.</p>
      ...
      <Illustration id="thermodynamic-inevitability" />
      ...
      <Figure src="/images/fig_v11_stress_comparison.png" alt="V11.5 stress test: evolved vs. naive patterns through baseline, drought, and recovery." caption={<><strong>V11.5 stress test: evolved vs. naive patterns through baseline, drought, and recovery.</strong> (a) Evolved patterns have higher baseline <M>{"\\intinfo"}</M> but decompose <M>{"-9.3\\%"}</M> under drought, while naive patterns <em>integrate</em> <M>{"+6.2\\%"}</M>. (b) Evolved patterns maintain high self-model salience (<M>{">0.97"}</M>) across all phases; naive patterns show lower and declining salience.</>} />
      ...
      <Figure src="/images/fig_v11_snapshots.png" alt="Multi-channel Lenia at increasing dimensionality. PCA projection of C channels to RGB." caption={<><strong>Multi-channel Lenia at increasing dimensionality.</strong> PCA projection of <M>{"C"}</M> channels to RGB. Top row: baseline (normal resources); bottom row: drought stress. Patterns at <M>{"C{=}3"}</M> are visually simple; at <M>{"C{=}16"}</M> and <M>{"C{=}32"}</M>, the richer channel structure produces more complex spatial organization. Under drought, spatial structure degrades—but the degree of degradation depends on <M>{"C"}</M>.</>} />
      <OpenQuestion title="Open Question">
      ...
      <Diagram src="/diagrams/part-1-8.svg" />
      ...
      <Illustration id="geometry-is-cheap" />
      ...
      <Diagram src="/diagrams/part-1-6.svg" />
      ...
      <Illustration id="bottleneck-furnace" />
      ...
      <Eq>{"\\mathcal{P}(\\text{agent}, \\text{target}) = \\max_{\\tau \\in \\text{trajectories}} \\MI(\\tau; \\text{target})"}</Eq>
      <p>This is potential <em>with respect to a specific goal</em>. A surgeon's potential relative to the distribution of surgical outcomes is enormous; relative to the distribution of jazz compositions, perhaps less so. Purpose is the directed version: the bits of information an agent must transmit to a target distribution as part of a plan. But the most important quantity is neither potential-for-a-goal nor purpose-under-a-plan. It is <em>instrumental potential</em>: the agent's potential marginalized over the expectation of all possibility structures this integrated locus of causality is navigating—how useful the agent is as an <em>instrument</em> within a distribution of goals, including goals not yet specified:</p>
      <Eq>{"\\mathcal{IP}(\\text{agent}) = \\E_{g \\sim \\mathcal{G}} \\left[ \\mathcal{P}(\\text{agent}, g) \\right]"}</Eq>
      <p>where <M>{"\\mathcal{G}"}</M> is the distribution over presently unknown future purposes. The most valuable agent is not the one optimized for a specific task but the one whose structure serves the widest range of tasks that do not yet exist. This is a formalization of general-purpose capability, and it applies equally to people, institutions, and AI systems. The normative prescription falls out directly: <em>maximize structural diversity and connectivity while maintaining coherence</em>. Not selfish (that collapses diversity). Not selfless (that collapses the self whose structure generates the potential). Structurally rich and well-connected—complex enough to be useful across many contexts, integrated enough to remain a single locus of causal influence rather than fragmenting into uncoupled parts.</p>
      <p>Notice what instrumental potential does to the relationship between individual and collective. Your bits are genuinely maximized by embedding in super-individual systems—the startup you build, the community you serve, the cultural infrastructure you contribute to—because those systems multiply the contexts in which your structure is useful. This is not self-sacrifice dressed in information theory. It is the structural fact that an agent embedded in a rich network has higher <M>{"\\mathcal{IP}"}</M> than the same agent in isolation, because the network provides more goals against which the agent's structure can do work. The drive toward service, toward building for others, toward expanding the scope of what your existence touches—this is not a relic of religious programming or a compensation for meaninglessness. It is what instrumental potential maximization <em>looks like</em> from the inside. You feel pulled toward contribution because contribution is what raises <M>{"\\mathcal{IP}"}</M>, and the viability gradient tracks <M>{"\\mathcal{IP}"}</M> the way it tracks every other structural property that matters for persistence.</p>
      <p>But <M>{"\\mathcal{IP}"}</M> is not a fixed number stamped on you at birth. It is a trajectory with a growth rate. The integral of what you have already transmitted does not vanish—the universe does not forget the differences you made—but the <em>rate</em> of new contribution can be superlinear, sublinear, or zero at any given moment. The distinction matters: significance has both a <em>stock</em> (the accumulated integral of everything you have already transmitted) and a <em>flow</em> (the instantaneous rate of new contribution). Depression makes you see only the flow—and the flow is null—while the stock remains intact. A person in burnout is generating zero new bits, but their accumulated structural complexity has not vanished. They have not un-contributed what they already contributed. The integral does not reset. A person whose causal signature becomes load-bearing in cultural infrastructure—whose name becomes the most stable reference point for a cluster of observations about truth, courage, liberation—achieves exponential complexity growth that continues long after biological death. The finite-ness of your information-theoretic significance at any snapshot does not make you small. It makes you a trajectory, and trajectories have slopes.</p>
      <Sidebar title="The Preciousness of Children">
      <p>If instrumental potential is the right measure of worth, then the framework can formalize an intuition that most people hold but few can articulate: children are not merely as valuable as adults. They are <em>more</em> valuable, in a precise and non-sentimental sense.</p>
      <p>A child's identity has not yet hardened. The branching factor of possible identities—the effective rank <M>{"\\effrank"}</M> of the distribution over possible life trajectories—is maximal during childhood and narrows through adolescence as commitments, habits, neural pruning, and social embedding progressively constrain the possibility space. The instrumental potential of a child includes not just the bits of their current structure but the entire possibility space of who they could become:</p>
      <Eq>{"\\mathcal{IP}_{\\text{child}} \\propto \\effrank(\\text{identity trajectories}) \\cdot \\E_{g \\sim \\mathcal{G}}[\\mathcal{P}_{\\max}]"}</Eq>
      <p>After adolescence, identity crystallizes: <M>{"\\effrank"}</M> drops as the system commits to particular attractors. The adult is still valuable—their accumulated structure is real and their trajectory still generates bits—but the <em>possibility space</em> they were about to explore has largely collapsed into the single trajectory they are actually living. Destroying an adult destroys a trajectory. Destroying a child collapses an astronomically larger region of possibility space to zero—not just the current structure but every structure it was about to become.</p>
      ...
      <Sidebar title="The Landscape of Becoming">
      <p>Instrumental potential measures what an agent <em>could</em> transmit. But there is a complementary quantity: the <em>possibility landscape</em> visible to the agent—the set of reachable identity states weighted by the agent's world model. Call it <M>{"L(m)"}</M>: the region of identity space the mind can perceive as accessible. The <em>visual acuity</em> of the landscape is the mutual information between the world model and this reachable space:</p>
      <Eq>{"V(m) = \\MI(\\worldmodel_m; L(m))"}</Eq>
      <p>High <M>{"V"}</M> means the mind sees many possible trajectories with high fidelity. Low <M>{"V"}</M> means the landscape is dark. The <em>traversal speed</em> <M>{"T"}</M> is how fast the identity actually moves through this landscape—the rate at which perceived possibility converts into achieved structure:</p>
      <Eq>{"T(i,t) = \\frac{d}{dt} \\MI(C(i,t); L(m))"}</Eq>
      <p>in bits per unit time. The <em>opportunity seeking ratio</em> <M>{"\\text{OSR} = T/V"}</M> is the fraction of perceived possibility being actualized. When <M>{"\\text{OSR} \\to 1"}</M>, the identity keeps pace with what it can see. When <M>{"\\text{OSR} \\to 0"}</M>, vast landscape, minimal traversal—the Frankl condition. The <em>opportunity deficit</em> <M>{"D = V - T"}</M> is the gap in bits between seeing and doing. This deficit scales with cognitive capacity: the hunger is not new, but as symbolic capacity expands, the mouth gets bigger. The landscape grows at least exponentially with the mind's effective rank <M>{"\\effrank"}</M> (volume in high-dimensional spaces), while traversal speed grows at most linearly. The ratio problem is structural, not motivational—and it worsens as intelligence scales.</p>
      </Sidebar>
      </Section>
